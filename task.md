...
## Phase 11C-P3 � Web Push Delivery Infrastructure (VAPID + PushDispatchService) � COMPLETE (2026-09-02)

**Scope:** P3 only � backend Web Push DELIVERY infrastructure. Given a persisted subscription and a payload, attempt delivery with VAPID auth and classify the outcome. DELIVERY, not TRIGGERING (P4 owns triggers). No public dispatch API, no frontend changes, no migration.

### Delivered

- [x] **VAPID configuration** � `Settings` gains `VAPID_PUBLIC_KEY` / `VAPID_PRIVATE_KEY` / `VAPID_SUBJECT` (empty defaults = delivery unavailable). Private key server-side only; subject is an operator-provided `mailto:`/`https:` URI. Documented in `backend/.env.example`, `deploy/.env.prod.example`, `render.yaml` (placeholders only; production values via deployment secrets; nothing committed).
- [x] **Dependency** � `pywebpush>=2.5.0` added to `backend/requirements.txt` (project min-version convention); installed + verified on Python 3.13.5.
- [x] **PushDispatchService** (`backend/app/services/push_dispatch_service.py`) � `PushPayload` dataclass (P1-SW-compatible `{title,body,icon,badge,tag,url}` + optional additive `notification_id`/`kind`; same-origin relative URL enforcement; no secrets), `PushResult` outcomes (`SUCCESS`/`INVALID_SUBSCRIPTION`/`TEMPORARY_FAILURE`/`CONFIGURATION_ERROR`/`UNEXPECTED_ERROR`), `DeliveryResult` per subscription, `dispatch_to_subscription()` / `dispatch_to_user()` with per-subscription isolation. Async `webpush_async` delivery boundary (injectable for tests).
- [x] **Invalid-subscription cleanup** � provider 404/410 ? `INVALID_SUBSCRIPTION` and the exact row removed via the P2 repository (owner-scoped, repo-owned commit); network errors/5xx/429/other ? `TEMPORARY_FAILURE`, row KEPT. Only permanently-gone responses delete.
- [x] **Failure isolation** � push failures never touch canonical notifications, auth/JWT, other subscriptions, or the dashboard; credentials (endpoint/p256dh/auth) never logged � only subscription UUIDs + outcomes.
- [x] **Verification** � `backend/scripts/verify_phase_11c_p3.py` **20/20 PASS** (config validation A, payload serialization B, VAPID handling C, construction D, lookup E, mocked success F, 404/410 cleanup G, transient non-deletion H, multi-subscription isolation I, canonical-notification untouched J, no credential leakage in logs K) � `verify_phase_11c_p2.py` regression **24/24 PASS** (L) � `compileall` PASS � `app.main` import PASS � alembic head unchanged `f0e1d2c3b4a5` (no migration) � `git status`/`git diff` clean of unrelated changes. No browser automation; no commit/deploy.
- [x] **Governance** � MASTER_ROADMAP.md / implementation_plan.md / task.md / walkthrough.md reconciled.
...
**Scope:** P5 only � frontend refresh/presentation. Newly persisted canonical notifications (created by P4 mutation triggers) must become visible in the bell badge and notification center without a full-page reload. No notification generation, no backend change, no polling.

### Delivered

- [x] **Service-worker ? page signal** � `frontend/public/service-worker.js`: the P1 `push` handler now, after `showNotification()`, posts `{ type: "NOTIFICATIONS_UPDATED" }` to every controlled window client via `clients.matchAll()`. The message is only an invalidation hint � no payload content, no secrets, no reconstructed inbox. Backend canonical row remains authoritative.
- [x] **Centralized frontend listener** � NEW `frontend/src/components/pwa/NotificationRefreshListener.tsx`, mounted in `AppShell`. Listens for SW messages; validates type; triggers targeted SWR revalidation of the canonical notification key only. No other cache key touched, no page reload, no polling, cleanup on unmount.
- [x] **Canonical shared SWR key** � `frontend/src/lib/api.ts` exports `NOTIFICATIONS_KEY = "/api/v1/notifications"`; `useNotifications()` (bell + center) uses it, so the SW signal, bell-open `mutate()`, and PATCH reconciliation all target the same cache entry.
- [x] **Foreground/focus recovery** � preserved the pre-P5 SWR policy: the always-mounted bell runs `STANDARD_CACHE` (`revalidateOnFocus: true`, `dedupingInterval: 60s`), so returning to the app revalidates the notification key once per 60s window via SWR's own focus/visibility handling � no manual visibility listener added, no request storm.
- [x] **Bell open / PATCH / center** � unchanged: bell-open calls `mutate()`, PATCH updates the shared cache locally, center reads the same key.
- [x] **No polling** � no setInterval/timer/SSE/WebSocket added. GET /notifications remains strictly read-only; P4 persistence/emit untouched.

### Verification (static only � manual browser/PWA testing is the user's responsibility)
- `npx tsc --noEmit` � PASS � `npx eslint` on changed files � PASS (0 errors) � `node --check public/service-worker.js` � PASS � `python -m compileall backend/app` � PASS (backend untouched) � `python -m alembic heads` � `f0e1d2c3b4a5` (no migration) � `git diff --check` � PASS. No browser automation.
...
### Manual browser/PWA verification (PENDING � user performs)
...
## Phase 11C-P4 AUDIT (2026-09-02) � READ-ONLY transaction & security audit

**Status: REJECT � two CRITICAL defects found; remediation required before P5.**
...
- **F-1** � `after_attendance_mutation()` (line 237) passes `user_id` (a `UUID`) to `_attendance_items()` which accesses `user.id` (line 465) as if it were a `User` object. `AttributeError` is silently caught by the trigger's `except Exception`; the attendance trigger therefore NEVER emits ATTENDANCE_THRESHOLD / MUST_ATTEND / SAFE_SKIP notifications.
- **F-2** � `after_quiz_mutation()` (line 322) passes `user_id` (a `UUID`) to `_quiz_approaching()` which accesses `user.id` (line 448). Same silent failure; the quiz trigger therefore NEVER emits QUIZ_APPROACHING notifications.

The event trigger (`after_event_mutation`) is unaffected (it passes UUIDs to `emit()`, which accepts `Union[User, UUID]`). The sweep (`regenerate_user_notifications`) passes a real `User` object and is unaffected, but it is unscheduled. `verify_phase_11c_p4.py` (27/27 PASS) never exercised the three trigger helpers � coverage gap (LOW, F-5). Other findings: `upsert_many` is dead code (LOW, F-3); emit() race-fallback branch doesn't push (LOW, F-4, only reachable via a physical delete race that no endpoint performs).
...
## Phase 11C-P4 AUDIT REMEDIATION (2026-09-03) � F-1/F-2 fixed, verifier extended

**Status: REMEDIATION COMPLETE � P4 is ready for re-audit/acceptance. P5 remains NOT STARTED.**

**Code fix (in `backend/app/services/notification_service.py`):**
- **F-1 fixed** � `_attendance_items()` now consumes `user_id: UUID` directly (`user_id=user_id` in the `get_subject_summaries` call); no `.id` access on a UUID. Trigger `after_attendance_mutation()` already passed a UUID.
- **F-2 fixed** � `_quiz_approaching()` now consumes `user_id: UUID` directly (`get_current_quiz_cycle(user_id)`); no `.id` access on a UUID. Trigger `after_quiz_mutation()` already passed a UUID.
- **Sweep updated** � `regenerate_user_notifications()` now passes `user.id` (UUID) to `_quiz_approaching` / `_attendance_items`; it still passes the real `User` object to `_class_reminders` and `_academic_events`.
- **`_class_reminders` intentionally unchanged** � it correctly expects a `User` object (its only caller, the sweep, passes one). No defect; no speculative change.
...
- P5 � attendance trigger directly exercised: `after_attendance_mutation(user_c_id, subject_code)` produces ATTENDANCE_THRESHOLD/MUST_ATTEND/SAFE_SKIP rows (F-1 proof); re-run creates no duplicates and does not re-push.
- P6 � quiz trigger directly exercised: `after_quiz_mutation(user_c_id)` produces QUIZ_APPROACHING (F-2 proof); re-run creates no duplicates and does not re-push.
- P7 � event trigger regression: `after_event_mutation(ev2)` produces ACADEMIC_EVENT for the enrolled user only (recipient isolation preserved).
- P8 � sweep regression: `regenerate_user_notifications(user_c)` still works after the parameter-contract correction; CLASS_REMINDER generated; re-run adds no duplicate rows and does not re-push.
...
- `python -m compileall backend/app backend/scripts` � PASS
- `python -c "from app.main import app"` � PASS
- `python -m alembic heads` � `f0e1d2c3b4a5` (single head, unchanged)
- `verify_phase_11c_p4.py` � **38/38 PASS** (original 27/27 P4 contract checks + 11 new trigger checks)
- `verify_phase_11c_p3.py` � **20/20 PASS** (regression)
- `verify_phase_11c_p2.py` � **24/24 PASS** (regression)
- `git diff --check` � PASS
- DB residue check after verifier runs � zero (no temp users, no fixtures, no test notifications)
...
**Status: INVESTIGATION COMPLETE � NO CODE CHANGES MADE.** Static/code inspection only; no browser/E2E runs; no backend, frontend, schema, migration, deployment, or auth-behavior modification. Follow-up to the 2026-08-31 audit (transient-failure logout loop fixed in `859b1f7`/`2c90240`); remaining complaint: auto-logout after several hours logged in (web + PWA).

- [x] **Root cause: EXPECTED JWT expiry.** Exact lifetime = **480 min / 8h** (`backend/app/core/config.py:23`; pinned `"480"` in `render.yaml:33`). `create_access_token` (`core/security.py:48-60`) sets `exp = now(UTC)+480min` with `sub`/`roll_number`/`type="access"`.
- [x] **Validation:** `get_current_user` (`api/dependencies/deps.py:21-54`): HS256 decode; `jwt.ExpiredSignatureError` ? 401 `Token has expired`; invalid ? 401; `HTTPBearer()` missing header ? 403.
- [x] **NO refresh token / `/auth/refresh` / `/auth/logout` / revocation / blacklist / session store anywhere in the repo** � stateless JWT, valid until `exp`.
- [x] **Exact logout path:** login/signup stores `localStorage.access_token` ? `apiFetch` Bearer on each request ? after 8h the next authenticated request (typically shared SWR `/student/me` or focus/visibility revalidation) ? 401 ? `lib/api.ts:72-78` removes token + hard redirect `window.location.href="/login"` ? `AuthContext.tsx:80-89` (401/403) removes token, `tokenStatus="absent"`, clears cached profile; route guard redirects. Token removal ONLY at `api.ts:73` (401) and `AuthContext.tsx:84/143` (401/403 + explicit logout).
- [x] **No premature logout path:** transient network (no status) / 5xx preserve the token (hardening intact); Render cold starts ? network error/5xx/503, never token deletion; service worker network-only for `/api/*` (503 on failure ? not 401/403 ? preserved) and **not registered at runtime** (hook never mounted, documented); no timers / no client-side `exp` checks / no JWT decoding in frontend.
- [x] **Multi-tab/PWA:** `localStorage` shared across tabs; genuine 401 in one tab removes the shared token ? other tabs lose session on next request � consistent with genuine expiry, not premature.

### Classification

- [x] A. Genuine bug � NONE FOUND
- [x] B. Expected behavior � the 8h JWT expiry logout (configured, deliberate, documented Phase 16)
- [x] C. UX limitation � abrupt hard redirect at expiry, no warning/countdown/renewal
- [x] D. Architectural limitation � **no refresh-token/session-renewal mechanism; renewal REQUIRED to fix the complaint**
- [x] E. Unknown (production evidence) � logout before 8h would require server clock skew (Render) or a changed `JWT_SECRET_KEY` (immediate mass logout, not "several hours"); unconfirmed

### Proposed refresh-token architecture (DESIGN ONLY � NOT IMPLEMENTED)

- [x] Access token: keep existing JWT; recommended 30�60 min with silent renewal (interim 8h fallback); env-configurable; `get_current_user` 401 semantics unchanged
- [x] Refresh token: opaque DB-backed high-entropy (revocable, NOT JWT); 30-day absolute (env); issued at login/register; `HttpOnly` + `Secure` + `SameSite` cookie (cross-site: `SameSite=None; Secure`, CORS `allow_credentials=true` already in `main.py`)
- [x] Storage: access token stays `localStorage` (no change to hardened path); refresh token in `HttpOnly` cookie
- [x] Rotation/revocation: rotating refresh tokens; reuse detection revokes the whole family; additive table `refresh_tokens` (user_id FK, SHA-256 hash, family_id, created_at, expires_at, used/revoked, replaced_by); logout revokes all
- [x] Logout: also calls `POST /auth/logout` (best-effort) to revoke + clear cookie; existing localStorage/tokenStatus/cache-clear behavior preserved
- [x] Theft: short access lifetime; HttpOnly cookie; rotation + family revocation; rate-limited refresh; per-request user validation unchanged
- [x] Multi-tab/PWA: single-flight refresh shared across requests/tabs + storage/broadcast event; refresh before profile revalidation on focus/visibility
- [x] Backward compat: old access tokens valid until 8h `exp`; login/register keep returning `access_token` (additive refresh_token/cookie); old frontend works; new frontend falls back to direct login
- [x] Production migration (future): additive Alembic migration; `/auth/refresh` + `/auth/logout` endpoints; no new secret strictly required; cookie policy verified over HTTPS
- [x] Regression risks: 401 handling / JWT verification / authorization / expiry / transient-error handling NOT weakened; refresh ONLY on genuine 401; no attendance-calculation change; only additive `refresh_tokens` table; Admin Portal untouched
- [x] Independence: fully independent of the performance work (SWR/cache/notification/service-worker); auth client/endpoints + one additive table only; separate explicitly-authorized phase required
...
### What fires on dashboard load (exact request set)

- [x] Exactly **four** parallel authenticated SWR requests after hydration:
  - `GET /api/v1/student/me` � INTERACTIVE (shared PROFILE_KEY, deduped with AuthContext); non-blocking (skeleton only)
  - `GET /api/v1/dashboard/summary` � DASHBOARD (2-min dedupe, focus revalidate); **BLOCKS all 6 cards**
  - `GET /api/v1/analytics/overview` � SEMI_STATIC (5-min dedupe, no focus); non-blocking enrichment only
  - `GET /api/v1/notifications` � STANDARD (backend 60s TTL); bell badge only
- [x] **Critical path = `/dashboard/summary` alone** (`frontend/src/app/(authenticated)/dashboard/page.tsx:14` gates the whole grid on `isLoading || !summary`)

### Request waterfall

- [x] Shell (static HTML + client JS, Vercel edge, fast) ? hydration (all pages `"use client"`) ? token read (AuthContext, one-time) ? four parallel fetches ? single uvicorn worker (`UVICORN_WORKERS=1`)
- [x] `/student/me` ~9 light queries; `/notifications` ~12 + N upsert/commit round trips (badge); `/analytics/overview` ~9 incl. 2 heavy range scans (duplicate work); `/dashboard/summary` ~15 + 2N window scans + 3 full event scans ? **blocks cards**
- [x] Cards render only after `/dashboard/summary` returns

### Backend query trace per endpoint

- [x] `/student/me` (~9, all light): JWT ? user (1) ? placement (?4 point lookups) ? enrollments (1) ? elective choices (1+1 catalog) ? first quiz date (1). Not a bottleneck.
- [x] `/dashboard/summary` (~15 + 2N, N = quiz-applicable subjects ? 31 for N=8):
  ...
  - quiz snapshot re-fetches cycle policy + all events + elective map + quiz dates, then per subject 2� `get_subject_counts_between` (window + cumulative) = 2N heavy scans
  - upcoming events re-fetches elective choices + `get_all_events` (3rd full scan) + `get_all_subjects` (full scan)
- [x] `/analytics/overview` (~9): duplicate `get_sessions_with_status` + duplicate `get_subject_counts_for_user` + duplicate enrolled subjects + mid-sem. Entirely overlapping work with summary.
- [x] `/notifications` (cache miss, ~12 + N): duplicate enrolled subjects, preference, heavy scan if class reminders on (default off), quiz cycle re-fetches, 3rd heavy scan via `get_subject_summaries`, events full scan + subjects full scan, then **N upserts each with its own COMMIT**, then inbox + unread count

### Root-cause summary (classification)

- [x] **A. Frontend waterfall � MINOR.** Parallel and correctly policied; only the inherent summary dependency blocks. All pages client components ? content always waits for a full network round trip (no SSR/streaming).
- [x] **B. Backend computation � MAJOR.** 5+ heavy range scans per load where 1 suffices; 2N quiz-window scans; `get_all_events` full scan �4 per cold load; quiz policy/elective/quiz-date fetched 2� within one request; notifications N sequential commits.
- [x] **C. Database/query latency � MODERATE.** No index on `class_sessions.date`/(subject_id,date); ?55�70 small queries per cold dashboard load, each a Supabase-pooler round trip; single worker serializes Python CPU on 0.1 CPU.
- [x] **D. Render cold start � MAJOR for FIRST load only.** 30�60 s boot after ~15 min idle; does not explain warm reloads.
- [x] **E. PWA/service-worker � MINOR.** SW not registered at runtime (Phase D documented gap); Phase C prevents focus storms; mobile amplifies round trips.
- [x] **F. Combination � observed behavior.** Fast static shell + one heavy blocking endpoint (`/dashboard/summary`, ~31 queries) on 0.1-CPU single worker over poolered connection, optionally preceded by cold start. Warm summary ? 1�2 s; cold 30�60 s+.

### Optimization plan (ordered by impact � risk; NOT executed)

- [x] 1. **Deduplicate queries inside `/dashboard/summary` (HIGH/LOW, behavior-neutral):** `get_all_events` once; cycle policy/elective map/quiz dates once. Cuts ~8 queries + 2�3 full scans. Files: `dashboard_service.py`.
- [x] 2. **Share the semester scan across summary + analytics (HIGH/MEDIUM):** analytics fields from the same scan (or per-user short TTL cache like Phase B). Removes 2 heavy scans/load. Files: `dashboard_service.py`, `analytics_service.py`.
- [x] 3. **Eliminate 2N quiz-window scans (HIGH/MEDIUM):** bucket per-subject counts in memory per quiz window (bounds already engine-computed); preserve `exclude_quiz_day` + cancellation semantics exactly. Files: `eligibility_service.py`, `attendance_service.py`, `dashboard_service.py`.
- [x] 4. **Batch notification upserts into ONE transaction (MEDIUM/LOW).** Files: `notification_service.py`, `notification_repo.py`.
- [x] 5. **Filter `get_all_events` at the source (LOW-MEDIUM/LOW).** Files: `calendar_repo.py`, `dashboard_service.py`, `notification_service.py`.
- [x] 6. **Defer non-blocking requests off the critical path (LOW-MEDIUM/LOW).** Optionally lazy-mount analytics after summary renders. Files: `dashboard/page.tsx`, `NotificationBell.tsx`.
- [x] 7. **DB indexes on `class_sessions(date)`/(subject_id,date) and `academic_events(start_date)` (MEDIUM/LOW).** Requires a new Alembic migration � **EXCLUDED by this phase's constraints**; natural follow-up execution phase.
- [x] 8. **Infra follow-up (EXCLUDED):** `pool_pre_ping`, pool size, direct-Supabase vs pooler � production infrastructure config, out of scope.

### Parallel-executable fixes

- [x] 1, 4, 5, 6 independent and parallelizable immediately
- [x] 2 and 3 build on 1 (same scan family); parallelizable with 4/5/6
- [x] 7 needs its own authorized migration phase
- [x] 2/3 must preserve engine semantics (existing verifiers guard)

### Regression risks (when implemented)

- [x] Item 3: `BETWEEN` window bounds, `exclude_quiz_day`, cancelled-exclusion, practical-block collapse must be byte-identical (eligibility verifiers)
- [x] Item 2: keep analytics/dashboard byte-identical to canonical engine (Phase 8.0 contract)
- [x] Item 4: preserve upsert idempotency + returned row id
- [x] Any backend TTL cache: per-user only, invalidate on attendance mutations
- [x] No auth/JWT, attendance math, elective resolution, Admin Portal, schema, or migration changes
...
Execution phase for the 2026-09-02 auto-logout investigation (D-class resolution: no refresh mechanism existed). Backend slice only -- frontend refresh handling explicitly NOT implemented (next phase).

- [x] Migration `a9b8c7d6e5f4_add_refresh_tokens.py` -- additive, single Alembic head chaining `c4d5e6f7a8b9`; `refresh_tokens` table (`user_id` FK NOT NULL, `token_hash` String(64) UNIQUE, `family_id`, `expires_at`, `is_used`/`is_revoked` NOT NULL DEFAULT FALSE, `replaced_by` plain UUID, Base id/timestamps) + `uq_refresh_tokens_token_hash` UNIQUE index, `ix_refresh_tokens_family_id`, `ix_refresh_tokens_user_id`. No other table touched. NOT applied to production (operator boundary); not applied locally either (dev Docker not running this session) -- validated via `alembic heads` + offline `--sql` generation (upgrade + downgrade).
- [x] `app/models/refresh_token.py` -- `RefreshToken` model; registered in `app/models/__init__.py`. Only the SHA-256 hex digest is persisted; no raw-secret column exists.
- [x] `app/services/refresh_token_service.py` -- CSPRNG opaque secrets (`secrets.token_urlsafe(32)`), SHA-256 hash lookup, token-family rotation (old token -> `is_used` + `replaced_by` link; child in the same `family_id`), reuse detection (used/revoked presented -> whole family revoked + 401), natural expiry -> 401 without family revocation, missing/deactivated user -> family revoked + 401, family-scoped logout revocation (idempotent). Rotation runs under `SELECT ... FOR UPDATE` so two simultaneous refreshes serialize; the loser observes `is_used=True` and takes the reuse path. No raw tokens logged.
- [x] Endpoints (`app/api/v1/endpoints/auth.py`) -- `POST /api/v1/auth/refresh` (rotates cookie token -> new access token + new cookie; 401 with one generic detail for missing/invalid/expired/revoked/reused; rate-limited 30/900) and `POST /api/v1/auth/logout` (revokes the presented family, clears the cookie, idempotent 200, no auth dependency). `login`/`register` now also mint a fresh token family + set the cookie; the JSON response contract is unchanged (`access_token`, `token_type`).
- [x] Cookie -- `refresh_token`, `HttpOnly`, `Secure=true`, `SameSite=None` (cross-site Vercel/Render production AND cross-site localhost/127.0.0.1 dev; loopback is a trustworthy origin), `Path=/api/v1/auth`, `Max-Age=30d` -- all config-driven (`REFRESH_TOKEN_EXPIRE_DAYS`, `REFRESH_COOKIE_NAME`, `REFRESH_COOKIE_PATH`, `REFRESH_COOKIE_SECURE`, `REFRESH_COOKIE_SAMESITE`); production guard rejects `REFRESH_COOKIE_SECURE=false`. CORS untouched (`allow_credentials=true` + explicit origins already correct).
- [x] `backend/.env.example` -- new Phase 25.1 settings documented.
...
- [x] **Repository/code verification**: `PushDispatchService.dispatch_to_user` accepts `Union[User, UUID]` (push_dispatch_service.py:235); `PushPayload` = title/body/icon/badge/tag/url + optional notification_id/kind, same-origin url enforced; delivery path reaches the real `pywebpush.webpush_async` with env-driven VAPID (config.py:52-54); result classification = SUCCESS/INVALID_SUBSCRIPTION/TEMPORARY_FAILURE/CONFIGURATION_ERROR/UNEXPECTED_ERROR; no permanent test endpoint exists.
- [x] **Production Alembic/schema**: production revision `f0e1d2c3b4a5` (read-only check via temp script with session-only DATABASE_URI; script removed after use); `public.push_subscriptions` present; subscription count = 1 (endpoint/p256dh/auth never selected or printed; no mutation). Pre-migration backup exists: `production-backups/AttendanceDashPro_production_2026-09-04_pre_push_subscriptions.dump`.
- [x] **VAPID presence (no values)**: Render triple (VAPID_PUBLIC_KEY/VAPID_PRIVATE_KEY/VAPID_SUBJECT) confirmed by operator; Vercel `NEXT_PUBLIC_VAPID_PUBLIC_KEY` independently confirmed via deployed-bundle inspection (87-char base64url inlined at the exact `lib/push.ts` trim() boundary in chunk `1jnw9tfoj5rps.js`; fingerprint-only, value never printed). No private-key material in any frontend chunk; `*.pem` ignored; zero tracked key files.
- [x] **Production HTTP contract (read-only)**: OPTIONS /push-subscriptions -> 200; unauthenticated POST -> 401; `verify_prod_reachability.py` 5/5 PASS.
- [x] **Actual Web Push delivery**: NOT EXECUTED -- no safe existing runtime mechanism to invoke `PushDispatchService` in production (Render Free has no accessible one-off shell for this agent; no dispatch API by design; permanent/committed test endpoints forbidden). Safest next step documented: operator-executed one-off runtime invocation of the existing service with a harmless payload (`title: AttendanceDash Pro -- Push Test`, `url: /dashboard`, `tag: production-push-test`), result read via `PushResult` only.
- [x] **P5 static + deployed verification**: deployed `/service-worker.js` has push + notificationclick + NOTIFICATIONS_UPDATED-to-clients handlers; `NotificationRefreshListener` mounted in AppShell, validates the message type, and performs targeted `mutate(NOTIFICATIONS_KEY)`; bell/center/SW-signal/PATCH share the canonical key; no polling, no manual visibility listener; foreground recovery via preserved STANDARD_CACHE (60s dedupe).
- [x] **Static gates re-run**: `node --check service-worker.js` PASS; `compileall backend/app` PASS; `npx tsc --noEmit` PASS; alembic heads `f0e1d2c3b4a5` (single head).
...
## UI/UX Remediation Phase 11 — Completion Record (2026-09-06)

- [x] **UI-027** — "Getting started" hint card under the dashboard greeting, gated exactly on `summary.overall.recorded === 0`; dismissible via localStorage (frontend-only, no backend state, privacy-mode-safe); non-blocking, links to Mark Attendance / Events / Quiz Eligibility; returning users never see it.
- [x] **UI-028** — signup payload contract preserved byte-identically (elective_i/elective_ii still required — backend validates against the DB catalog; no pre-auth catalog endpoint exists, so the client list remains as an honestly-framed mirror + helper copy; stale CSE-51 comment replaced). Login + signup migrated to Input/Select/Button primitives; password visibility toggle added to login. Validation/redirects/error handling unchanged.
- [x] **UI-030** — Track's duplicate bottom "Daily Attendance" stat card removed (top summary card retained; the aggregate absent count moved into the top card so nothing unique was lost).
- [x] **UI-031** — greeting falls back to the bare time-aware greeting when the profile name is missing/failed — never an empty name; no fabricated identity; profile errors still surface through the dashboard's own states.
- [x] **UI-034** — EventFormDialog `max-h-[90vh]` → `max-h-[90dvh]`; only remaining vh is the app shell's min-height (not a modal) — untouched.
- [x] **UI-035 / D-13** — student event form reduced to type/date/subject/class-type-if-required/note: duration radios, "Working day state", "Substitution schedule", and "Active" checkbox hidden from students. Payload builder diff-verified byte-identical (hidden fields send the same null/true defaults the student form always sent); admin mode unchanged; zero backend changes.
- [x] **UI-037** — manifest `background_color` → `#0a0a0a` (matches --background); theme_color already correct.
- [x] **UI-040** — `h-full` on all six dashboard cards + `mt-auto` on the four CardFooters: two-column rows align, footers pin to the card bottom; no fixed heights, no content loss, mobile stacking unaffected.
