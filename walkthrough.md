...
Notification row COMMITTED
       ?
PushDispatchService (best-effort, isolated)
  only for NEWLY created rows
```

### Key change: GET /notifications is now read-only

Pre-P4: `NotificationService.get_notifications()` generated projections from four sources (class reminders, quiz approaching, attendance items, academic events), upserted them all, then served the inbox. P4 eliminates this entirely: `get_notifications()` reads only the persisted inbox from the DB (or the 60s TTL cache). Notification rows are created only by mutation triggers (attendance marking, event create/update, quiz schedule change) and by the explicit callable sweep. The historical 11A/11B verifier assertions of generation-on-read are intentionally superseded.

### Files changed

| File | Change |
|---|---|
| `backend/app/services/notification_service.py` | Read-only `get_notifications()`; `emit()` boundary; `_notify_push`; trigger helpers `after_attendance_mutation`/`after_event_mutation`/`after_quiz_mutation`; `regenerate_user_notifications` sweep; kind deep-link map |
| `backend/app/repositories/notification_repo.py` | `try_create` (race-free `INSERT � ON CONFLICT DO NOTHING RETURNING`), `get_by_occurrence_key` |
...
| `backend/app/services/admin_quiz_service.py` | Post-commit quiz schedule triggers (`_notify_quiz_users` helper) |
| `backend/scripts/verify_phase_11c_p4.py` | **NEW** verifier (27/27 PASS) |
| `MASTER_ROADMAP.md`, `implementation_plan.md`, `task.md`, `walkthrough.md` | Governance reconciliation |

### Emission boundary

`NotificationService.emit()` is the single canonical write path:

- **New row:** `NotificationRepository.try_create` uses `INSERT � ON CONFLICT DO NOTHING RETURNING id` � atomic, race-free, never duplicates. The row is committed; `_notify_push` dispatches a best-effort Web Push via `PushDispatchService` (or the injected mock).
- **Existing row:** refresh `message`/`subject_code`/`subject_name`/`updated_at` in place, preserving `date`, `is_read`, `is_dismissed`, `created_at`. **No re-push** � a repeated trigger or sweep of the same fact never sends a duplicate push.
- `PushPayload` is derived from the canonical Notification row: `title="AttendanceDash Pro"`, `body=row.message[:400]`, `url=kind-based deep link` (`/history`, `/tools/events`, `/tools/quiz-schedule`, `/dashboard`), `notification_id=str(row.id)`, `kind=row.kind`. No secrets, no JWTs, no subscription keys.

### Mutation triggers

| Trigger | Location | Emitted kinds | Recipient resolution |
|---|---|---|---|
| Attendance marked | `AttendanceService.record_attendance` (post-commit) | ATTENDANCE_THRESHOLD, MUST_ATTEND, SAFE_SKIP | The affected user's resolved subject (elective slot ? concrete choice, not anchor) |
| Event created/updated | `EventService.create_event`/`update_event` (post-commit) | ACADEMIC_EVENT | Concrete-subject events ? enrolled users of that subject only (elective isolation: BCS-058 event ? BCS-058 enrollees only, never BCS-055 students); elective-slot events ? slot choosers + anchor fallback; global events ? all users; past/inactive events skipped |
| Quiz schedule created/updated | `AdminQuizService.create_quiz_schedule`/`update_quiz_schedule` (post-commit) | QUIZ_APPROACHING | Enrolled users of the schedule subject + (if elective slot) slot choosers |

All triggers:
- Fire **after** the domain mutation commits (transaction ordering: domain ? notification ? push).
- Are **fully isolated** � they never raise to the caller. Failures are logged with safe metadata.
- Use lazy (function-level) imports of `NotificationService` to avoid circular imports.

### Sweep (callable, NOT scheduled)

`NotificationService.regenerate_user_notifications(user)` regenerates all four canonical notification families for a user via `emit()`:
- `_class_reminders` � gated by the `class_reminders` preference
- `_quiz_approaching` � next-upcoming quiz cycle
- `_attendance_items` � per-subject threshold/optimizer state
- `_academic_events` � upcoming active events (top-4, enrolled-scoped, elective-resolved)

This sweep is callable but **not scheduled** � no cron/Celery/Redis/APScheduler was added. Time-dependent regeneration (primarily CLASS_REMINDER) is explicitly deferred from scheduling. P5 or operations may invoke the sweep when needed.
...
```
domain mutation commits ? NotificationService.emit ? try_create commits ? push dispatch (best-effort)
```

A push failure never rolls back or deletes the canonical notification row. A domain mutation failure never reaches `emit()`. The notification is persisted in its own transaction after the domain commit.

### Idempotency / duplicate prevention

- `UNIQUE(user_id, kind, occurrence_key)` DB constraint prevents duplicate rows.
- `emit()` on an existing row refreshes without re-pushing � a repeated trigger produces no duplicate push.
- `try_create` uses `ON CONFLICT DO NOTHING`: concurrent emits are race-free (only one wins the insert; the loser reads the existing row and refreshes).

### Verification performed

- `python -m compileall backend/app` � **PASS**
- `python -c "from app.main import app"` � **PASS** (no circular imports)
- `python scripts/verify_phase_11c_p4.py` � **27/27 PASS** (A: read-only GET, B: emit creates row, C: push-once, D: duplicate suppression, E: push-failure isolation, F: zero-subscription persistence, G: VAPID-missing persistence, H: P3 cleanup decoupling, I: multi-subscription isolation, J: recipient isolation, K: read-only no-write GET, L: PATCH read-state regression)
- `python scripts/verify_phase_11c_p3.py` � **20/20 PASS** regression
- `python scripts/verify_phase_11c_p2.py` � **24/24 PASS** regression
- Alembic head � `f0e1d2c3b4a5` (unchanged, no migration)
- `git diff --check` � **PASS** (no whitespace errors)
- **No browser automation, Playwright/Cypress/Selenium, or E2E testing was run**

### Explicit non-goals (NOT in P4)

- No frontend changes (P5 owns bell/in-app refresh).
...
**ZERO changes to existing tables.** The migration graph is unchanged (alembic head `f0e1d2c3b4a5`). No existing table, row, view, or constraint is modified. The verifier's test rows are created and removed within the same run.

### Remaining work (P5, NOT STARTED)

- **P5 � bell/in-app refresh improvements:** e.g., a modest `refreshInterval` on `useNotifications` for real-time freshness (parallelizable with Phase 26 performance work).

**HARD STOP after P4 � no commit, no push, no deploy, no browser automation.**

**Phase 11C-P4 AUDIT (2026-09-02): REJECTED.** Two CRITICAL defects found (F-1/F-2). **REMEDIATION COMPLETE (2026-09-03)** — F-1/F-2 fixed, verifier extended to 38/38 PASS, P2/P3 regressions green. See task.md for full findings.


---

## Phase 11C-P5 — Notification Bell / In-App Refresh — Walkthrough

**Status: COMPLETE (2026-09-03).** P5 of the five-phase Web Push delivery plan. Frontend refresh/presentation only: newly persisted canonical notifications become visible in the bell badge and notification center without a full-page reload. Event-driven revalidation, NOT polling. **No backend change, no migration, no scheduler.**

### Scope
- Service-worker invalidation signal to controlled clients.
- Single centralized frontend listener (NotificationRefreshListener).
- Canonical shared SWR key (NOTIFICATIONS_KEY) for bell, center, SW signal, PATCH.
- Foreground/focus recovery via the preserved STANDARD_CACHE focus policy (one deduped request per 60s).
- Bell-open mutate and PATCH local cache reconciliation (unchanged).

### Files changed
| File | Change |
|---|---|
| `frontend/public/service-worker.js` | push handler posts NOTIFICATIONS_UPDATED to controlled clients |
| `frontend/src/components/pwa/NotificationRefreshListener.tsx` | NEW centralized listener |
...
### Request behavior
- Normal mount: one GET /api/v1/notifications from the always-mounted bell (existing).
- Push while foreground: SW message -> one targeted mutate (deduped by SWR).
- Background -> foreground: SWR focus revalidation, one request per 60s window (STANDARD_CACHE).
- Bell open: mutate() (existing).
- Mark read / dismiss: local cache update (existing, non-optimistic).
- **Polling: none.**

### Verification performed
- `npx tsc --noEmit` PASS - `npx eslint` changed files PASS - `node --check public/service-worker.js` PASS - `python -m compileall backend/app` PASS - `python -m alembic heads` `f0e1d2c3b4a5` - `git diff --check` PASS. **No browser automation run; manual PWA testing is the user responsibility (PENDING).**

### Manual verification checklist (user)
1. Foreground push -> bell badge refreshes without reload; center shows the row.
2. Center open -> new push refreshes the open list from canonical GET.
3. Background -> foreground -> one targeted refresh catches up.
4. Bell open with stale cache -> latest rows appear.
5. Mark read -> badge updates; dismiss -> list/badge synchronized.
6. Offline/transient failure -> refresh recovers without logout.
7. Installed PWA -> repeat key checks.

### Known limitations
- CLASS_REMINDER remains sweep-only and unscheduled (separate future phase; not P5).
- Real Web Push requires valid production VAPID config + a persisted browser subscription.
- Multi-tab: each tab revalidates its own SWR cache; tabs that miss the SW message catch up on focus/visibility.
- Manual E2E still pending (user).
...
## Phase 11C -- Production Web Push Verification & Hardening -- Walkthrough (2026-09-04)

**Status: COMPLETE (verification-only).** The production Web Push chain was verified as far as can be safely verified without new production code, new endpoints, schema mutation, or credential exposure. Actual push DELIVERY was not executed (no safe runtime mechanism exists -- see below). Zero application code changed.

### What was verified, with evidence

1. **Code contracts (static)**: `dispatch_to_user(User | UUID)` (push_dispatch_service.py:235-247); `PushPayload {title, body, icon, badge, tag, url}` + optional `notification_id`/`kind` with same-origin `url` enforcement (push_dispatch_service.py:56-98); `_default_send` calls the real `pywebpush.webpush_async` with env-driven VAPID + TTL 3600 + timeout 10s (push_dispatch_service.py:149-162); `PushResult` five-way classification (push_dispatch_service.py:30-45); `NotificationService.emit` -> `_notify_push` -> `PushDispatchService` best-effort, never raises (notification_service.py:195-220). A direct dispatch creates NO canonical notification row -- safe for infrastructure verification.

2. **Production Alembic**: `f0e1d2c3b4a5` (head) -- read via a temporary read-only script using a session-only `DATABASE_URI`; the script printed ONLY the revision, table existence, and count and was deleted immediately after (never committed). Production was previously `a9b8c7d6e5f4`; the P2 migration was applied 2026-09-04 with the pre-migration backup `production-backups/AttendanceDashPro_production_2026-09-04_pre_push_subscriptions.dump` in place.

3. **push_subscriptions persistence**: table present; SUBSCRIPTION COUNT = 1. The row was created by the operator's browser (POST /push-subscriptions -> 200 after the migration; the pre-migration 500 was the missing-table symptom). Endpoint/p256dh/auth were NEVER selected or printed; no row was modified. Local dev DB cross-check (read-only): `f0e1d2c3b4a5`, table present, 0 rows -- consistent (the browser subscription targets production).

4. **VAPID presence (values never revealed)**:
   - Render: `VAPID_PUBLIC_KEY` / `VAPID_PRIVATE_KEY` / `VAPID_SUBJECT` -- presence confirmed by the operator in the dashboard.
   - Vercel: `NEXT_PUBLIC_VAPID_PUBLIC_KEY` -- presence verified INDEPENDENTLY of the dashboard: the deployed login/dashboard chunks were scanned for an 87-char base64url constant (the P-256 public key length); exactly one match exists, in chunk `1jnw9tfoj5rps.js`, at the exact minified form of `VAPID_PUBLIC_KEY = (process.env.NEXT_PUBLIC_VAPID_PUBLIC_KEY || "").trim()` from `frontend/src/lib/push.ts` (assignment + `.trim()` + `length > 0` guard all confirmed). Inspection was fingerprint-only (first/last 4 characters reported; the value itself was never printed, stored, or committed). The private key appears in no frontend chunk. `*.pem` is gitignored; zero tracked key files.

5. **Production HTTP contract (read-only probes)**: OPTIONS /api/v1/push-subscriptions with the Vercel origin -> 200; unauthenticated POST -> 401 (auth boundary intact); `verify_prod_reachability.py` 5/5 PASS (Vercel /login 200; production API URL inlined; Render /health 200; CORS preflight; invalid login -> 401 not 500).

6. **P5 behavior (static + deployed artifact)**: the DEPLOYED `/service-worker.js` contains the `push` listener (defensive `parsePushPayload` -> `showNotification`), the `notificationclick` listener (same-origin re-validated focus + navigate / openWindow), and `signalClientsNotificationsUpdated()` posting `{type: "NOTIFICATIONS_UPDATED"}` to all controlled window clients (includeUncontrolled) inside `event.waitUntil(Promise.all([...]))`. In the app: `NotificationRefreshListener` is mounted in AppShell (AppShell.tsx:31), validates `event.data.type === "NOTIFICATIONS_UPDATED"`, and calls `mutate(NOTIFICATIONS_KEY)` only. `NOTIFICATIONS_KEY = "/api/v1/notifications"` (lib/api.ts:34) is the single shared key for the bell (`useNotifications`, useApi.ts:470), the center, the SW signal, and PATCH reconciliation. No polling, no manual visibility listener; foreground recovery rides the preserved STANDARD_CACHE focus policy (revalidateOnFocus, 60s dedupe). The open/revalidation behavior from Phase 11D is intact.

### What was NOT verified, and why

- **Actual Web Push delivery (Phase D)**: not executed. There is no safe existing runtime mechanism to invoke `PushDispatchService` in production: Render Free exposes no one-off shell to this agent, no dispatch API exists by design (P4 triggers push only through `NotificationService.emit`, which would create a canonical notification row -- notification-generation verification, explicitly out of scope), and creating a permanent (or committing a temporary) test endpoint is forbidden. The VAPID private key was never placed in the repository or local `.env`.
- **Safest next step (operator)**: a one-off, ephemeral runtime invocation in the deployed environment (Render one-off job / shell) executing the existing service against the persisted subscription, e.g. `PushDispatchService(db).dispatch_to_user(<user_uuid>, PushPayload(title="AttendanceDash Pro -- Push Test", body="Production Web Push delivery is working.", url="/dashboard", tag="production-push-test"))`. It performs zero schema writes and no canonical-notification writes; the outcome is read from the returned `PushResult` (SUCCESS / INVALID_SUBSCRIPTION / TEMPORARY_FAILURE / CONFIGURATION_ERROR / UNEXPECTED_ERROR) and nothing else is printed. INVALID_SUBSCRIPTION legitimately removes the stored row (existing P3 contract) -- the operator should be aware before running it.

### Verification gates re-run (2026-09-04)

- `node --check frontend/public/service-worker.js` PASS
- `python -m compileall backend/app` PASS
- `npx tsc --noEmit` PASS (frontend)
- `python -m alembic heads` -> `f0e1d2c3b4a5 (head)` (single)
- `verify_prod_reachability.py` 5/5 PASS
- No browser automation, no expensive test suites, no load on production beyond the read-only probes above.

### Manual checklist remaining (owner)

1. Foreground push -> OS notification displayed; bell badge increments without reload.
2. Background push -> notification displayed; on return, bell catches up via focus revalidation (one deduped request).
3. notificationclick -> existing tab focused + navigated to the payload url (new tab when closed).
4. Notification center after a push -> canonical rows visible.
5. Optional operator one-off delivery test (above) -> observe SUCCESS + the notification; INVALID_SUBSCRIPTION would also prove row cleanup.

### Governance & hygiene

- MASTER_ROADMAP.md, implementation_plan.md, task.md, walkthrough.md synchronized with this evidence (no duplicate historical plans created).
- Temporary verification script created then removed; not committed. No `.env` modified. No commit/push/merge/deploy performed. Git tree clean except the operator's untracked backup dump (`production-backups/AttendanceDashPro_production_2026-09-04_pre_push_subscriptions.dump`) left for the owner to track or ignore.
- HARD STOP after this verification slice.
---

## UI/UX Remediation Blueprint — Walkthrough (PLANNING ONLY, 2026-09-05)

- **Inputs:** audit at `docs/UI_UX_AUDIT_STUDENT_APP_2026-09-05.md` (40 findings, six systemic root patterns). Repository re-inspection during planning: frontend primitives/hooks/pages (unchanged since audit), backend contract boundaries (`app/api/v1/endpoints/` — `notifications.py` PATCH per-item idempotent, `preferences.py` full-object PUT), and band constants (`app/engines/attendance_engine.py:11-13`: target 75, SAFE ≥80, WATCH ≥60 — the weekly card's hard-coded 80/60 coincidentally matches; remediation centralizes constants, does not change values).
- **Output:** `docs/UI_UX_REMEDIATION_BLUEPRINT_2026-09-05.md` — 12-phase dependency-ordered plan (Phase 0 decision gate), all 40 findings mapped, 13 product decisions (D-01..D-13) documented with options/recommendations and NOT silently chosen, design-system consolidation table (canonical primitive vs duplicate vs retain), navigation/IA proposal with options (no route changes), feedback/error model, responsive strategy per band, accessibility plan, terminology standard, file impact map (~40 files), risk matrix (no VERY HIGH; two HIGH-impact cells guarded: `lib/api.ts` additive-only, nav IA decision-gated), manual verification checklist (10 sections, viewport matrix 320–1920), static verification gates per phase, protected-systems list.
- **New planning-time discoveries (kept separate from the 40):** A-1 orphaned `SubjectLaboratoryView.tsx`; A-2 dead hooks `useCalendarDay`/`useSubjectSummary`; A-3 EventRow "Calendar" link lacks date deep-linking.
- **Verification performed during planning:** static only — grep usage checks (orphan/dead code), backend endpoint contract reads, band-constant read, compiled-CSS dead-token check (from audit, re-confirmed). No browser automation. No code modified: `git status` clean except these governance/plan documents.
- **Not done:** any implementation phase; any backend/schema/migration/auth/push change; any commit/push/deploy. Remediation is NOT started — Phases 0–12 all remain unchecked tasks.
- HARD STOP after blueprint production.

---

## UI/UX Remediation Phase 1 — Walkthrough (COMPLETE, 2026-09-05, implementation)

- **Authorized scope:** blueprint Phase 1, findings UI-001 + UI-002 ONLY (the authorizing prompt explicitly excluded UI-007/404-error UI from this pass; recorded as still-open).
- **Discovery before editing:** fresh grep confirmed repository unchanged since audit — 31 dead-token usages in 12 files; accent inventory split into 13 visible usages (fixed) vs legitimate menu hover/focus surface usages in `UserMenu.tsx:58` and `dropdown-menu.tsx` (retained, shadcn-correct). `.next` stale build regenerated during verification.
- **UI-001 approach:** existing-token migration preferred over new tokens (per prompt + blueprint preference) — `globals.css` untouched, zero new tokens. `bg-surface2` semantics ("slightly elevated subtle surface") map onto the app's existing `bg-muted` + opacity idiom already used by SubjectAttendanceCard chips/Progress track; `bg-surface` card-like panel → `bg-card`; `text-text2` → `text-muted-foreground/70`. Skeleton fix is at the class level: `cn("bg-muted", "bg-muted/50")` → tailwind-merge keeps `bg-muted/50` (valid, compiled) instead of the old invalid `bg-surface/50` stripping the only valid background. Component API/animation/radius/sizing untouched.
- **UI-002 approach:** per-consumer semantic mapping, not mechanical replace — selection/highlight/link/icon semantics → `primary` (the design system's interactive/highlight color: links are already `text-primary`, focus `ring` = primary); event-presence indicators (dot, count, legend) → `primary` (informational highlight; distinct from today via shape/position and from selection via weight); feedback SUGGESTION badge → `warning` (only unclaimed semantic slot among BUG=destructive/QUESTION=primary/PRAISE=success). Hover surfaces on `accent` retained — changing `--accent`'s value would have repainted every menu hover surface (rejected as regression).
- **Incident during implementation:** two multi-line Edit operations on `laboratory/page.tsx` flex-matched and dropped one `)` from the `))}` map closers (lines 261/479). Caught immediately by `tsc --noEmit`, repaired with disambiguating context, and confirmed absent from the final diff (file differs from pre-phase state only in its 8 intended class lines).
- **Verification evidence:** `npx tsc --noEmit` PASS; ESLint PASS on all 14 changed files; `NEXT_PUBLIC_API_URL=<CI placeholder> npm run build` PASS (25/25 pages); compiled chunk CSS inspected — ZERO `bg-surface*`/`text2` selectors; present: `.bg-muted\/30`, `.bg-muted\/50`, `.bg-card`, `.bg-primary\/10`, `.bg-primary\/15`, `.ring-primary`, `.border-primary`, `.text-primary`, `.text-muted-foreground\/70`, `.bg-warning\/10`, `.text-warning`; `git diff --check` PASS; `git diff -- backend/` empty; final frontend diff = 42 insertions / 42 deletions (class swaps only, manually reviewed line by line).
- **NOT done (deliberate):** UI-007 404/error UI; Phases 2–12; any backend/schema/migration/auth/push change; any navigation/terminology/date/pct/settings/notification/destructive-confirmation change (D-01..D-13 remain future work); no commit/push/deploy.
- **Manual verification remaining (user):** skeleton visibility on Laboratory (3 tabs), Quiz Eligibility, Subjects grid; calendar event dot + count, selection ring + tint, today circle, all four legend swatches; DayDetail event left borders + substitution line; EventRow/profile/quiz link + icon tint; avatars (UserMenu, ProfileModal, AdminShell, profile page); laboratory tab track + progress track; quiz inactive tabs + info panel; OverallAttendanceCard forecast qualifier tone; SUGGESTION badge on admin Feedback list.
- HARD STOP after Phase 1 — awaiting explicit authorization for the next phase.

---

## Local Dev Backend Port Migration 8080 -> 8300 — Walkthrough (COMPLETE, 2026-09-05)

- **Root cause.** Windows reserves 8027-8126 as an excluded TCP port range (Hyper-V/WinNAT); `uvicorn --port 8080` died at bind with WinError 10013. 8300 was verified free before the move (`netstat -ano | findstr :8300` no listener; `netsh interface ipv4 show excludedportrange protocol=tcp` no coverage of 8300).
- **Discovery before editing.** A repo-wide `8080` sweep classified every occurrence first: dev-runtime truth lived in exactly six files (`start-dev.ps1`, `stop-dev.ps1`, `frontend/.env.local`, `frontend/.env.example`, `frontend/src/lib/api.ts`, and a comment in `backend/app/core/config.py`). Everything else was history (legacy `npx serve . -p 8080` static-frontend era docs, dated investigation/audit reports) or a generated artifact (`repomix-output.xml`).
- **Consistency by construction.** `start-dev.ps1` routes its port-availability check, ownership check, bind-wait loop, error messages, and the final status/URL summary through the single `$backendPort` variable — only the assignment changed, so a mixed 8080/8300 state is structurally impossible. `stop-dev.ps1` kills by port + process-name pattern, so `8300` + `python` matches the new listener exactly.
- **Frontend URL choice.** `127.0.0.1:8300` everywhere (not `localhost:8300`) because the backend binds IPv4 loopback only and `docs/login_connectivity_investigation.md` recorded `localhost` resolving to `::1` and missing the bind. `.env.local` mattered most: `NEXT_PUBLIC_API_URL` overrides `api.ts`'s `DEV_API_URL` fallback, so leaving it at 8080 would have kept the frontend pointed at the dead port.
- **Pre-existing defect fixed en route (disclosed).** `stop-dev.ps1` was BOM-less; Windows PowerShell 5.1 decodes BOM-less scripts as ANSI, and its UTF-8 box-drawing/em-dash characters decode into parser-breaking sequences (`Parser::ParseFile` → "Missing closing '}'" at line 49). The HEAD copy fails identically, so this predates the migration and is the same failure class fixed for `start-dev.ps1` in Phase 26.7. Fix = restore the UTF-8 BOM (a 3-byte prefix; zero logic bytes touched). Both launchers now AST-parse clean under the PS 5.1 parser. (PowerShell 7 users were never affected.)
- **Deliberately untouched.** CORS (frontend origins on 3100; the backend's own port is not a CORS concept), the refresh-cookie SameSite model (both ends loopback, unchanged), the production stack (Render PORT injection, Dockerfile 8000 default, `render.yaml`, `docker-compose.prod.yml`), `docker-compose.yml` (postgres 55432 only), all application logic/schema/engines, and Windows port exclusions/WSL networking.
- **Static verification performed.** PowerShell AST parse of both launchers (PS 5.1 parser) PASS after the BOM restore; `py_compile` on config.py PASS; post-change repo-wide grep: zero `8080` in any executable/config path; `git diff` on the launchers reviewed line-by-line (em-dashes/box characters intact).
- **Not done (deliberate).** No runtime testing (the operator runs the stack manually); no browser/E2E/test-suite runs; no rewrites of dated reports; no Hyper-V/WSL changes; no dependency changes.
- **Operator notes.** Restart the Next.js dev server so `NEXT_PUBLIC_API_URL` re-inlines as `http://127.0.0.1:8300`; `start-dev.ps1` will display Backend `http://127.0.0.1:8300` and API `http://127.0.0.1:8300/api/v1`. If 8300 is ever occupied by an unrelated process, the launcher exits naming the owner rather than silently picking another port.
- HARD STOP after the port migration.

---

## UI/UX Remediation Phase 2 — Walkthrough (COMPLETE, 2026-09-05, implementation)

- **Discovery:** repo state reconciled before editing — Phase 1 fixes intact; no toast dependency/primitive existed; only student-scope `window.alert` was events deactivation; `not-found/error.tsx` absent; ErrorState also consumed by 9 admin pages (hence additive-only API); SW update branch console-only; analytics `isError` unused in dashboard. **User's coexisting local changes found and preserved:** dev-port migration 8080→8300 across `backend/app/core/config.py`, `lib/api.ts` DEV_API_URL, `start-dev.ps1`, `stop-dev.ps1` — pre-dating Phase 2, excluded from phase diff accounting, untouched. (First api.ts edit was rejected stale precisely because of this; re-read picked up the user's version and layered only the session-expiry block.)
- **Key decisions:** (1) local toast implementation over a new dependency — matches minimality rule; (2) toasts are supplementary: existing inline success/error indicators (Settings, Track cards, NotificationCenter) intentionally untouched to avoid scope creep; (3) ErrorState kept its existing red visual treatment (color-token normalization is UI-012, out of scope) — only the action API was added; (4) analytics note is warning-toned (non-critical semantics) with a Retry that hits only the analytics key; (5) SW banner reload is explicitly user-initiated to match the worker's no-skipWaiting design; (6) session flag lives ONLY in the genuine-401 permanent-failure branch — 403/5xx/network paths unchanged.
- **Incident-free notes:** one TS error in the new toast (optional `variant` index) fixed by making `ToastItem.variant` required; the login effect's sync setState was reworked to a deferred (timeout-0) pattern to satisfy `react-hooks/set-state-in-effect`; the two `err: any`/apostrophe lint errors in the touched login file (pre-existing on HEAD) were cleaned as hygiene.
- **Verification results:** `npx tsc --noEmit` PASS; ESLint on 15 changed files: 13 clean; history ×2 set-state-in-effect + api.ts location-assign warning proven pre-existing on HEAD (stash-verified via `git show HEAD:`); `npm run build` PASS 25/25 pages; `git diff --check` PASS; targeted greps: 0 `window.alert` in student scope/components, analytics + SW + session wiring present, zero API payload/contract changes, zero destructive-confirmation (Phase 3) additions.
- **Manual browser verification still required (user):** forced render error → boundary with Try again/dashboard; unknown URL → 404 page; block analytics → dashboard note + targeted retry; block summary/history/subjects/track → ErrorState retry actually revalidates; deactivate-failure toast (no native alert); SW update banner after a deploy (Refresh applies, dismiss sticks); expired session → login notice; toast stack/auto-dismiss/keyboard/mobile position above bottom nav; dark-only appearance consistent.
- HARD STOP after Phase 2 — awaiting explicit authorization for Phase 3. No commit/push/deploy.

---

## UI/UX Remediation Phase 3 — Walkthrough (COMPLETE, 2026-09-05, implementation)

- **Discovery:** flows matched Phase 2 state exactly — mark-all wired directly to mutation (single-tap risk confirmed), lab Trash2 buttons mutated immediately (two destructive actions, not more; no others found), events inline Confirm/Cancel intact with Phase 2 toasts and no alerts; no existing ConfirmDialog; user's pre-existing dev-port changes still in tree (preserved).
- **What changed (UI flows):** (1) Track — "Mark all present" now opens a confirmation showing the real pending count; Confirm runs the existing Promise.allSettled bulk mutation; result toasts are honest (full success with count / partial "X of Y" warning / total-failure error) and the inline error banner remains for failures; per-class Change behavior untouched (mutation stays reversible, hence default not destructive variant). (2) Laboratory — delete-record and deactivate-experiment open destructive ConfirmDialogs that name the experiment (number/title/subject); run() returns success so "Record deleted"/"Experiment deactivated" toasts fire only on genuine success; failures keep the inline banner; the confirm handler guards a missing record instead of the old `!` assertion. (3) Events — interaction retained; a ref guard now makes the inline Confirm immune to double-submission.
- **Confirmation behavior:** initial taps never mutate; dialogs lock (spinner, disabled Cancel/Confirm) until the mutation settles and then close; Escape/outside-click inherited from the Dialog primitive; focus management inherited (no nested dialogs created).
- **Feedback behavior:** toasts supplement, not replace — inline error surfaces retained where they were the better medium (lab mutations); counts shown only when actually known from results (never fabricated).
- **Verification performed:** tsc PASS; ESLint on the 4 changed files PASS with 0 findings; production build PASS 25/25 pages (CI placeholder URL); `git diff --check` PASS; static searches: 0 student-scope `window.alert`/`alert(` (admin-portal alerts remain out of scope as residue), ConfirmDialog imports the shared ui/dialog, no API payload/endpoint changes in the Phase 3 diff, zero diffs under components/notifications, migrations/backend untouched by this phase (backend tree still carries only the user's pre-existing config.py port line).
- **Important decisions:** default-vs-destructive variant per action recoverability; dialog closes after settle (caller-owned feedback); run() boolean return added (additive — other callers unaffected); events guard is a ref (no re-render cost, no visible loading change to the retained inline flow).
- **Manual verification remaining (user):** bulk — tap button → dialog with count; Cancel = no mutation; Confirm = exactly one bulk run; button cannot double-submit; honest feedback on partial failure if reproducible. Laboratory — Delete → confirm → exactly one deletion (Cancel = nothing); Deactivate experiment → confirm → one state change. Events — Deactivate → inline Confirm/Cancel; Confirm → success toast; failure shows toast, never alert(). Keyboard — open confirmation, Tab between Cancel/Confirm, Enter/Escape behavior, focus returns sensibly. Mobile (~360px) — dialog not clipped, buttons tappable, no background interaction, reachable above bottom nav.
- HARD STOP after Phase 3 — awaiting explicit authorization for Phase 4. No commit/push/deploy.

---

## UI/UX Remediation Phase 4 — Walkthrough (COMPLETE, 2026-09-05, implementation)

- **Discovery:** nav data was duplicated across TopNav (8 items + admin) and MobileBottomNav (3 tabs + More list) with the audited conflicting labels; "View Strategy" lived only on the dashboard Attention card pointing at the Track route; both lab pages carried the old titles; /tools/quiz-schedule confirmed to exist; DropdownMenuItem supports the Base UI render-prop pattern already used by Button.
- **Navigation architecture now:** `navItems.ts` defines every destination once (label/icon/mobile title) with role helpers and two group lists (mobile tabs; More destinations). TopNav derives three render bands — lg+ full, md–lg primary+More dropdown, <md hidden — and MobileBottomNav derives its tabs and More sheet from the same data, so labels can never drift again. Active-state convention (exact pathname match) is retained everywhere; correct because student nav routes have no nested children.
- **Responsive behavior:** at 768–1023 the row holds Home/Attendance/History + More▾ (secondary: Mark Attendance, Lab Experiments, Quiz Eligibility, Calendar, Events, +Feedback for admins) — the audited overflow is structurally impossible; at ≥1024 the full inline nav appears per the original design; below 768 the bottom bar + More sheet own navigation. The More trigger tints and sets aria-current="true" while a secondary route is open, on both desktop (md band) and mobile.
- **Track/Laboratory terminology (D-01):** nav labels, Track page h1 + error-branch PageHeader ("Mark Attendance"), and Laboratory page PageHeader ("Lab Experiments") updated; internal identifiers, hook names, comments, and URLs untouched; attendance/lab functionality unchanged.
- **View plan (D-02):** dashboard "Attention Required" CTA now reads "View plan" and opens /tools/quiz-schedule — closing the attention→plan loop the audit flagged as a dead end.
- **Mobile page titles (D-04):** the header shows the current page title centered for non-home routes (e.g. "Calendar", "Quiz Eligibility", "Attendance History"); the brand wordmark yields below sm while a title is displayed; Home/dashboard intentionally keeps the wordmark + greeting with no duplicate entry.
- **Verification performed:** tsc PASS; ESLint on the 6 changed files PASS (0 findings); production build PASS 25/25 pages (CI placeholder URL); `git diff --check` PASS; static searches confirm no "View Strategy"/"Track Attendance"/old nav labels remain in frontend/src, the shared module is consumed by both navs, all 10 routes still exist, notifications dir has zero diffs, backend untouched by this phase (only the user's pre-existing config.py port line remains), and no Phase 5 container refactor leaked.
- **Manual verification remaining (user):** 768px — no overflow, More dropdown opens with the five secondary destinations, active tint while inside them; 1024px — transition to full nav (admins: check 9 items fit); large desktop — full nav labels; Track route shows "Mark Attendance" with unchanged functionality; Laboratory shows "Lab Experiments"; dashboard "View plan" opens Quiz Eligibility; mobile 360/390 — More highlights inside secondary routes, header shows the page title, no duplicate Home, no overflow; direct navigation between secondary routes keeps More active.
- HARD STOP after Phase 4 — awaiting explicit authorization for Phase 5. No commit/push/deploy.

---

## Phase 4 Correction — Walkthrough (2026-09-05)

- **Reported regression:** at lg+ the new labels wrapped ("Mark Attendance", "Lab Experiments", "Quiz Eligibility") and the user name ("Aditya Tiwari") wrapped — navbar became two lines tall.
- **Diagnosis before editing (as required):** the header is a flex row; nav items, the More trigger, and the right control group were all shrinkable flex items without whitespace-nowrap, and the nav carried min-w-0 (which licenses shrinking below content width). The D-01 labels added ~+75px intrinsic width vs the old labels, so at the widths the old design fit, flexbox began compressing items and inline text wrapped — including the identity name.
- **Fix applied (TopNav.tsx, UserMenu.tsx):** every nav item and the More trigger now carry `shrink-0 whitespace-nowrap` and `px-2.5` (was `px-3`); both navs dropped `min-w-0` and gained `shrink-0` (lg band also dropped `lg:gap-1.5` in favor of `gap-1`); the header row tightened `gap-4`→`gap-3`; the right control group is `shrink-0`; the identity span is `whitespace-nowrap`. Net: ~78px reclaimed vs ~75px label growth — the lg+ band returns to the old compact width budget while keeping the correct D-01 labels. Nothing else moved: breakpoints, labels, mobile titles, More active state, navItems.ts all unchanged; no overflow-hidden/truncation/max-width hacks on desktop; no font-size change.
- **Verification:** tsc PASS; ESLint (TopNav, UserMenu) PASS; production build PASS 25/25 pages; `git diff --check` PASS; diff review confirms the correction touches only the class-level changes above (the lone `truncate` in the diff is the pre-existing Phase 4 MOBILE page title, intentionally untouched).
- **Manual re-verification (user):** desktop at 1024/1100/1280/1440/1600+ — every label on one line, user name on one line, no horizontal scrollbar, active states correct, full nav at lg+; 768–1023 — More band unchanged; mobile — page titles, More active state, bottom nav unchanged.
- HARD STOP — awaiting manual verification; no Phase 5; no commit/push/deploy.

---

## UI/UX Remediation Phase 5 — Walkthrough (COMPLETE, 2026-09-05, implementation)

- **Discovery:** AppShell already owned the intended contract (`mx-auto w-full max-w-5xl p-4 pb-28 md:p-6 lg:p-8`) — the duplication was entirely page-side: 17 root occurrences across 9 files, each carrying `px-4 py-N sm:px-6 lg:px-8` plus a max-w (5xl == shell width; 7xl unreachable inside 5xl; 4xl/2xl narrower-than-shell). Dashboard was already clean (`w-full`).
- **Contract now:** AppShell owns width/centering/horizontal padding; pages own vertical rhythm (`py-*`, `gap-6`, `space-y-6`, header `mb-8` — all preserved) and legitimate local widths (Track `max-w-2xl`, Quiz/Feedback/Profile `max-w-4xl`, each with `mx-auto`, retained per blueprint §7 and now constraining content INSIDE the shell column).
- **What changed:** only root-container className strings on 9 page files (17 occurrences). No wrapper DOM added/removed; no vertical values touched; no AppShell/nav/primitive changes; no new abstractions or tokens.
- **Expected geometry delta (not a regression):** affected pages lose the double horizontal inset — mobile 64→32px per side... precisely: inset per side drops 64→32 on mobile and 128→64 on desktop; content width grows by the previously wasted padding. Track/Quiz/Feedback/Profile keep their narrower reading columns centered inside the shell.
- **Verification performed:** tsc PASS; ESLint 9 changed files — 8 clean, only the 2 history set-state-in-effect errors proven pre-existing on HEAD (Phase 2); production build PASS 25/25 pages (CI placeholder); `git diff --check` PASS; post-change container audit — zero `px-4 py-`/`sm:px-6`/`lg:px-8` shell-duplication patterns in app/(authenticated), remaining root max-w-* exactly the 4 classified local constraints; AppShell container classes byte-identical; navigation/navItems untouched this phase; notifications/contexts zero diffs; backend still only the user's pre-existing config.py port line.
- **Manual verification remaining (user):** all 10 authenticated pages at 360/390/768/1024/1280/1440/1600+ — centered content, consistent single-inset margins, no double padding, no horizontal scrollbar, expected widening on previously double-padded pages (Events/Calendar/History/Subjects/Laboratory), Track/Quiz/Feedback/Profile reading columns intact, Phase 3 confirmation dialogs correctly positioned, Phase 4 nav bands + mobile titles/More-active intact, Calendar geometry/selection/day-detail intact.
- HARD STOP after Phase 5 — awaiting explicit authorization for Phase 6. No commit/push/deploy.

---

## UI/UX Remediation Phase 6 — Walkthrough (COMPLETE, 2026-09-05, implementation)

- **Inventory before refactoring (as required):** 11 native selects across 5 files in 4 accidental height systems (h-8 filter strings ×2 files, h-10 sm:h-8 dialog string, h-9/sm:h-7 settings, h-9 laboratory); shared Input fixed at h-8 (32px, below the app's 40px mobile control convention); Button foundation already canonical at h-10 sm:h-8; 50 text-[10px] occurrences in 15 files; humanizeEventType/classTypeLabel duplicated byte-identically in EventRow + DayDetail; student-scope DialogFooter users = EventFormDialog (form, not confirmation) + the primitive.
- **Slice 1 (heights):** canonical = h-10 sm:h-8. Fixed the SHARED Input primitive rather than adding abstractions; created ui/select.tsx (native select — semantics and native picker preserved — styled to match Input; data-slot; w-full overridable via className, which Settings/Laboratory use as w-auto beside labels). All 11 selects migrated; the four ad-hoc class strings deleted; date inputs across history/events/dialog reduced to [color-scheme:dark] (primitive supplies height/ring/bg); Track date input simplified. Two transient slips during the pass (missing Select import in laboratory page; EventRow/DayDetail type-import fallout) were caught by tsc and fixed immediately.
- **Slice 2 (confirmations):** audit clean — no equivalent modal confirmations to migrate; ConfirmDialog API sufficient as-is; EventRow inline Confirm/Cancel untouched per D-11.
- **Slice 4 (duplicates):** exactly one consolidation — the two identical label helpers into eventRules.ts (the existing shared event-domain module), consumers re-pointed; deliberately NOT consolidated: status.ts classTypeLabel (different return contract), date chips (style drift only), date formatters (Phase 7/D-09 will canonicalize in one place once the shared location exists — actually the formatters were left fully in place), QUIZ_LABELS vs CYCLES, remaining inline empties (Phase 2 documented variations).
- **Slice 3 (typography):** uniform 10→11px floor (readable + intentional; density hierarchy preserved — no text-xs/sm changes), leading-none added to the 14 h-4 overflow-hidden badges to prevent 0.5px clipping at the larger size; h-5 badges fit without changes; NotificationBell counter (leading-none already) included; mobile bottom-nav text-[0.65rem] labels deliberately retained as compact tab microcopy.
- **Verification performed:** tsc PASS; ESLint 20 changed files → 19 clean + 2 history findings proven pre-existing on HEAD (Phase 2); production build PASS 25/25 pages (CI placeholder); git diff --check PASS; four post-audits all PASS (0 native selects / 0 text-[10px] / single humanizeEventType / ConfirmDialog + inline confirm intact); diff review — zero onChange/handler/payload lines changed, zero new "use client" (boundaries preserved), zero Phase 7 formatting/vocabulary leakage; ids/labels/htmlFor carried over identically with the migrated elements.
- **Manual verification remaining (user):** controls (History/Events filters, Event form, Lab subject select + ingest, Settings week select, Track date input) sized consistently at 360/390/768/1024/1280/1440 — mobile controls now 40px; 11px labels/badges legible with no clipping; disabled/loading states intact; Phase 3 confirmations (bulk mark-all, lab delete/deactivate) and D-11 inline event confirm unchanged; Phase 1 calendar indicators/skeletons, Phase 2 ErrorState/toasts/banner, Phase 4 navbar bands + mobile titles, Phase 5 container widths all unregressed.
- HARD STOP after Phase 6 — awaiting explicit authorization for Phase 7. No commit/push/deploy.

---

## UI/UX Remediation Phase 7 — Walkthrough (COMPLETE, 2026-09-05, implementation)

- **Inventory:** 2 raw status-enum badge sites (Overall/Attention cards); 4 label systems for attendance states (status.ts Attended/Missed, Track Present/Absent, History Present/Absent, Laboratory raw backend strings); 7 locale-dependent/Intl date sites (SubjectAttendanceCard device-locale, QuizEligibilityCard en-US, EventRow/DayDetail en-US formatters, laboratory en-US, SubjectLaboratoryView en-US with a pre-existing UTC-parse day-shift risk on negative-offset devices, ProfileModal en-GB); percentage split between formatPct (round), fmtPct (toFixed(1)) and fmtIntPct (toFixed(0)); 7 user-facing dev-copy targets; 1 wall-of-text quiz intro; 1 unexplained `· Np`.
- **Vocabulary decisions (per approved D-07/D-08):** Present/Absent for states and actions; "attended N of M" kept as prose; Healthy/Watch/Critical for the legacy bands via attendanceStatusLabel (backend values untouched); Health set labels already canonical in SubjectAttendanceCard — now consistent BY VALUE across surfaces.
- **Threshold preservation proof:** WeeklyAttendanceCard 80/60 replaced by WEEKLY_BAR_SAFE_PCT/WEEKLY_BAR_WATCH_PCT — same numbers, sourced from and documented against backend/app/engines/attendance_engine.py's SAFE_BAND_PCT (target+5=80) / WATCH_BAND_PCT (target−15=60). The semantic-integrity diff audit found zero numeric threshold changes elsewhere.
- **Date handling:** formatDateMedium uses the existing toDate (`YYYY-MM-DD` → `T00:00:00` local), so academic date-only strings keep their calendar date — no UTC shift; SubjectLaboratoryView's old `new Date(iso)` UTC parse (day-shift risk) was replaced by the local-parse canonical as a side effect of consolidation. Date SEMANTICS untouched everywhere (display only).
- **Percentage proof:** every change replaces a formatter call on the same data field (`pa.current_practical_pct`, `eligibility.lecture_pct/tutorial_pct/average_pct`, subject headline/detail) — raw values, thresholds (70/75), and the CriterionRow toFixed(1) calculation detail untouched; formatDelta "+2.3 pts" intentionally kept (signed delta, not a percentage display).
- **Copy:** all UI-010 targets resolved with student-facing wording; uncertain/borderline cases reviewed and retained (Events "The server enforces…" card, admin branch of EventFormDialog description) and reported here rather than silently changed.
- **Verification performed:** tsc PASS; ESLint on all 22 touched files PASS (two pre-existing signup/login-area lint items fixed as touched-file hygiene: err:any typing, apostrophe escape); production build PA
SS 25/25 pages (CI placeholder); git diff --check PASS; post-audits all PASS (raw enums 0, locale dates 0 with remaining Intl classified as month/time labels, toFixed remaining = whole-number + sanctioned quiz-calc, dev copy 0 user-facing, text-[10px] 0, pending indicator resolved); semantic-integrity diff audit clean (thresholds/arithmetic/payloads/comparisons/date-parsing unchanged).
- **Manual verification remaining (user):** dashboard status badges show Healthy/Watch/Critical with correct colors; Today card and Laboratory badges show Present/Absent; summaries read naturally; dates on Dashboard/Calendar/History/Events/Quiz/Laboratory are deterministic English with NO day shifts; percentages whole-number everywhere except the Quiz "View Calculation" detail (one decimal preserved); login/signup wording matches the roll-number credential model; quiz intro readable without parsing formulas; weekly rows show "N pending"; responsive wrap check at 360/390/768/1024/1280 for longer status labels and date strings.
- HARD STOP after Phase 7 — awaiting explicit authorization for Phase 8. No commit/push/deploy.

---

## Phase 7 Correction — Walkthrough (2026-09-05)

- **Reported regression:** Phase 7's D-10 interpretation rounded calculated attendance percentages to whole numbers, hiding meaningful precision (72.2% → 72%, 87.5% → 88%, etc.).
- **Root cause:** D-10 was applied to ALL percentages instead of only categorical/summary displays; the attendance-value formatters (SubjectAttendanceCard toFixed(1), QuizEligibilityCard toFixed(1), Laboratory toFixed(1)) were collapsed into the rounding formatPct.
- **Fix:** NEW canonical `formatPct1` (toFixed(1), null → "—") in lib/date.ts; precision restored on exactly the regression set — SubjectAttendanceCard headline + detail rows (fmtIntPct deleted; Lecture/Tutorial blocks move to shared formatPct, still whole as they always were), QuizEligibilityCard lecture/tutorial/average + CriterionRow (local fmtPct now delegates to formatPct1 — one decimal across the whole card, identical to pre-Phase-7 output), Laboratory practical stat. Decimal formatting now lives in exactly one shared helper.
- **Deliberately whole-number (pre-existing presentation, not regressions):** history summary, dashboard Overall/Forecast, AttentionRequiredCard current/forecast, WeeklyAttendanceCard weekly/best/needs-attention, criterion "required N%" labels.
- **Verification:** tsc PASS; ESLint (4 changed files) PASS; production build PASS 25/25 pages (CI placeholder); git diff --check PASS; diff audit shows zero calculation/threshold/payload/query lines — display-only formatter swaps on identical data fields.
- **Manual re-verification (user):** subject cards show one-decimal attendance (72.2% style); quiz rows and View Calculation show one decimal; Laboratory practical Attendance stat one decimal; dashboard/history remain whole numbers; no other visual changes.
- HARD STOP — awaiting manual re-verification; Phase 8 not started; no commit/push/deploy.

---

## UI/UX Remediation Phase 8 — Walkthrough (COMPLETE, 2026-09-05, implementation)

- **Discovery:** NotificationCenter held a single `pendingId: string | null` whose non-null state disabled Read/Dismiss on EVERY row (the audited global lock); rows came from the canonical NOTIFICATIONS_KEY SWR hook gated on open; read/dismiss used the per-item PATCH returning the updated NotificationItem with cache updates applied only after genuine 2xx; the empty state used CalendarDays; the bell shares the same key; NotificationRefreshListener already targets the canonical key on the SW message. The PATCH endpoint is idempotent and accepts is_dismissed:false — making a genuine dismissal undo possible without backend changes.
- **Pending model:** replaced the global lock with `pendingIds: string[]` (rows with an in-flight PATCH) + `markAllPending: boolean`. Row buttons disable only for their own row (plus during mark-all); unrelated rows remain fully interactive; early-return guards prevent same-row double-submission and row-vs-mark-all races; two different rows may mutate concurrently — safe, independent, idempotent PATCHes.
- **Mark all read (D-12):** button above the list (visible when unreadCount > 0; disabled while mark-all runs or any row is pending) triggers a SEQUENTIAL for..of over the type-narrowed unread ids, each awaiting the existing PATCH `{is_read:true}` and updating only that row's cache entry after its genuine 2xx. Outcome honesty: all succeed → success toast with count; some fail → warning toast + inline error banner naming the failed count; all fail → error toast + banner with no cache mutation (rows stay unread = server truth). No bulk endpoint, no migration, no burst.
- **Dismiss undo (D-11 portion):** dismissal remains immediate with no confirmation; a new 8s info toast offers "Undo", which calls the SAME endpoint with `{is_dismissed:false}` and then revalidates the canonical key once (row returns in server order). Undo failure surfaces an error toast; if the toast expires first, the row simply stays dismissed (server truth). Mark-read undo deferred deliberately: the row remains visible after read, so nothing is lost — documented rather than built.
- **Toast primitive:** extended additively with an optional `action {label, onClick}` (clicking runs the handler and closes the toast) — no new dependency; existing variants, timers, aria-live roles, and dismiss button unchanged.
- **UI-039:** empty-state icon corrected to Bell (matches the entry point); CalendarDays retained for the ACADEMIC_EVENT kind badge; layout/typography untouched.
- **Verification performed:** tsc PASS (three string|null notification_id type errors caught and fixed: undo guard, type-predicate filter for the unread list, narrowed rowId in the row map); ESLint on both changed files PASS; production build PASS 25/25 pages (CI placeholder); git diff --check PASS; audits — sequential PATCH confirmed, zero backend bulk-route hits, payloads limited to the existing is_read/is_dismissed contract, canonical key + NotificationRefreshListener + NotificationBell intact, no polling, no new visibility listeners, backend tree unchanged apart from the user's pre-existing config.py port line, Phase 1–7 work unregressed (build + targeted greps).
- **Manual verification remaining (user):** mark-all with all/partials/no failures (toggle backend); unrelated rows stay clickable while one row spins; mark-all disabled during row actions and vice versa; dismiss → Undo restores the row in server order within 8s; dismissing the last row shows the Bell empty state and Undo restores it; bell badge count tracks every operation; push-triggered badge refresh still works; 360px layout — long messages, buttons above/below the fold, no horizontal overflow; undo toast expires gracefully.
- HARD STOP after Phase 8 — awaiting explicit authorization for Phase 9. No commit/push/deploy.

---

## UI/UX Remediation Phase 9 — Walkthrough (COMPLETE, 2026-09-05, implementation)

- **Discovery:** week_starts_on was read ONLY by SettingsModal (usePreferences → GET/PUT /api/v1/student/preferences, backend default MONDAY per preferences.py); CalendarGrid hard-coded Sunday-first via getDay() leading blanks and Sun-first headers; auto_mark_present was write-only storage with NO consumer and NO attendance automation anywhere in the backend (auto_mark exists only in the preference model/repo/endpoint); the Settings toggle implied functionality the system does not have; the info box would become stale once D-05 landed.
- **D-05 implementation:** CalendarGrid gained `weekStartsOn?: WeekStart` (default MONDAY). Monday-start rotates WEEKDAY_LABELS/WEEKDAY_SHORT left by one (Mon..Sun / M,T,W,T,F,S,S) and computes leading blanks as (getDay()+6)%7; Sunday-start keeps the original arrays and getDay(). The grid then fills sequentially, so every date lands under its true weekday column regardless of preference; selection, today, event dots, and class-count labels are per-date and therefore move WITH their dates. No date parsing was added or changed (Phase 7 local T00:00:00 handling intact). The calendar page consumes the preference through the shared usePreferences SWR key (deduped with Settings — one logical request) and falls back to MONDAY while loading (matches the backend default).
- **D-06 handling:** honest deferral. The toggle remains visible and still persists through the unchanged full-object PUT (SAVED PREFERENCE ≠ CLAIMED FUNCTIONALITY); a caption under the row states "Automatic attendance marking is not currently active — the preference is saved with your account."; no engine/service/repository/API modification and no invented future behavior. The bottom info box was rewritten to stay truthful: week start controls the calendar layout; auto-mark is saved but does not mark attendance yet.
- **Verification performed:** tsc PASS; ESLint on the 3 changed files PASS; production build PASS 25/25 pages (CI placeholder); git diff --check PASS; audits — rotation/blank formulas verified for both starts (e.g. Tuesday start: Sunday-start 2 blanks, Monday-start 1 blank), preferences draft/payload lines unchanged in the diff (0 payload touches), no new Date() constructions in CalendarGrid, backend/engines/services/migrations untouched (backend tree still only the user's pre-existing config.py port line), Phase 1–8 work unregressed.
- **Manual verification remaining (user):** toggle week start in Settings → calendar re-renders Mon-first vs Sun-first; verify a month starting on Friday/Saturday/Sunday under BOTH starts (leading blanks correct); selected date, today ring, event dots, and class counts stay on the correct dates; DayDetail matches the tapped date; settings save shows existing feedback; auto-mark caption visible under the toggle; 360px settings + calendar layouts clean.
- HARD STOP after Phase 9 — awaiting explicit authorization for Phase 10. No commit/push/deploy.

---

## UI/UX Remediation Phase 10 — Walkthrough (COMPLETE, 2026-09-06, implementation)

- **Discovery:** the quiz cycle selector (`tools/quiz-schedule/page.tsx`) declared `role="tablist"`/`role="tab"`/`aria-selected` with no `tabpanel`, no `aria-controls`, no ids, and no arrow-key handling — the audited UI-020. Its real interaction model is a filter: a click re-renders the entire card list below (loading skeletons, empty state, and per-subject cards — there is no single panel element to label). The laboratory section switcher (`laboratory/page.tsx`) is three buttons inside a `<nav>` using `aria-current="page"` — but they toggle in-page views (attendance/experiments/activity), not routes (UI-033). Touch-target audit of student-scope controls found the Button/Input/Select foundation already compliant everywhere it is used (h-10/sm:h-8 pair; calendar cells min-h-12; bottom nav min-h-14; dialog/sheet closes and calendar navs icon-sm=40px mobile; UpdateBanner/TrackSessionCard compliant since Phase 12B) — the violations were page-level overrides and two custom controls: History/Events reset buttons (`className="h-7"` stripped the mobile override), Settings switches (h-5 w-9 = 20px), the quiz pills (h-8 = 32px), lab section buttons (32px), toast dismiss (22px, below the 24px floor), and NotificationBell (36px mobile). The toast container already carried full `role="status"/"alert"` + `aria-live` semantics from Phase 2; the Track page's mutation-error region had none.
- **Semantic model chosen:** both selectors are exclusive view/state filters rendered as native buttons — NOT tab interfaces (no panels) and NOT navigation (no routes). Per the blueprint's recommended option, both dropped their incorrect ARIA (`tablist`/`tab`/`aria-selected` on quiz; `aria-current="page"` + `<nav>` on lab) for `role="group"` with an accessible name + `aria-pressed` on each button. Tab/Enter/Space behavior is native; no custom keyboard logic, no roving tabindex, no new primitives. Focus is not lost on switch (buttons persist; only the content below re-renders).
- **Touch-target changes (hit-area only, visuals preserved):** quiz pills `h-8` → `h-10 sm:h-8`; lab buttons `py-1.5` → `h-10 sm:h-8` (both: 40px mobile, byte-identical 32px desktop, standard focus ring added); History + Events reset `h-7` → `text-xs sm:h-7`; Settings switches `h-5 w-9` → `h-6 w-11` with thumb `size-4` and checked offset recomputed 18px → 24px (44×24, blueprint-prescribed, same visual language); toast dismiss `p-1` → `p-1.5` (26px); NotificationBell `p-2` → `p-2.5 sm:p-2` (40px mobile). NotificationCenter dismiss verified 40px mobile — no change (blueprint verify-only).
- **Live regions:** `aria-live="polite"` added to the Track mutation-error banner. Contrast statically verified (#94a3b8: 8.4:1 background / 7.6:1 card / ≈7.9:1 chips — AA and AAA at 11px captions).
- **Deferred (recorded, not implemented):** EventRow's four admin-gated buttons keep `h-7` (28px mobile) — admin-render-only, outside the blueprint's Phase 10 file list; TopNav desktop links ≈30px (pointer UI, hidden on mobile, meets the 24px floor); QuizEligibilityCard `size="xs"` Retry 24px desktop-only (foundation design). Two pre-existing `react-hooks/set-state-in-effect` ESLint errors in the History page's untouched effects were noted and left alone.
- **Verification performed:** tsc PASS (0 errors); ESLint PASS on all changed files (the 2 History errors pre-date this phase — flagged lines are in effects this phase never touched); production build PASS 25/25 pages (CI placeholder URL); `git diff --check` PASS; targeted ARIA searches — zero `role="tab"`/`tablist`/`aria-selected` and zero `aria-current`-on-buttons in student scope, `aria-pressed` on both selectors, `aria-live` on Track error + toast; residual `h-7` scan shows only the deferred EventRow buttons; no shared primitive modified, so Phase 1–9 surfaces are structurally untouched (build-verified).
- **Manual verification remaining (user):** keyboard-only pass — Tab reaches the quiz selector and lab tabs, Enter/Space activates, focus ring visible, focus retained across switches; screen-reader smoke test — "pressed/not pressed" announced on both selectors, Track mutation failure announced politely; 360px tap ergonomics — quiz pills, lab tabs, Settings switch, History/Events reset, bell, toast dismiss; desktop visual spot-check — pills/switches/bell byte-identical to prior rendering.
- HARD STOP after Phase 10 — awaiting explicit authorization for Phase 11. No commit/push/deploy.

---

## UI/UX Remediation Phase 11 — Walkthrough (COMPLETE, 2026-09-06, implementation)

- **Discovery:** each finding was re-validated against the code before editing. (1) The signup electives are NOT a removable frontend default: `POST /auth/register` declares `elective_i`/`elective_ii` as required strings and validates them against the active semester's `subjects.elective_slot` catalog (auth.py Phase 22.3/23.5), surfacing 422 details the page already renders; no pre-auth catalog endpoint exists, so a client list is the only possible source without an unauthorized backend change. (2) The Track page's bottom "Daily Attendance" stat card repeated Present/Absent/Pending; the aggregate absent count was unique to it. (3) The dashboard greeting interpolated an empty first name on profile failure. (4) EventFormDialog was the only modal still capping at `90vh`. (5) The event form rendered "Working day state", "Substitution schedule", duration radios, and the "Active" checkbox to students; students are create-only (`canEdit={isAdmin}` on every EventRow, "Add Event" unconditional) and every student-creatable type defaults to single-day. (6) Only TodayAttendanceCard carried `h-full`; the Card primitive is `flex flex-col`, so stretched short cards left their footers floating mid-card (the ragged-bottom perception). (7) Manifest `background_color` #0F172A vs app `#0a0a0a`.
- **UI-027 strategy:** no backend onboarding state exists and none was invented. A single hint card renders under the greeting when `summary.overall.recorded === 0` (the blueprint's gate) and until dismissed; dismissal persists in localStorage with try/catch (privacy mode → per-visit); the lazy state initializer is SSR-safe and the block never renders during prerender (SWR data arrives client-side), so there is no hydration mismatch. Non-blocking, one card, three real links, icon-sm dismiss per the Phase 10 touch standard.
- **UI-028 handling:** payload preserved byte-identically (diff-audited: zero request-body lines changed). The client option list stays as the honest mirror of the deployed catalog with an accurate comment and a helper line ("options reflect the current semester's catalog; the server verifies your selection"). Both pages adopted Input/Select/Button primitives (selects gain `[color-scheme:dark]`; heights join the h-10/sm:h-8 control family); login gained the signup-style password toggle. No auth architecture, validation, redirect, or storage behavior changed.
- **UI-030 handling:** bottom stat card removed; the uniquely-useful aggregate absent count was folded into the top summary card ("N absent" when > 0) — the audit's duplication is gone without losing information.
- **D-13 contract handling:** the student form hides fields; it does not weaken them. The `handleSubmit` payload builder is untouched (diff-verified), so hidden fields submit the same defaults the student form always produced (`is_working_day: null`, `substitution_schedule_override: null`, `active: true`). Admin mode keeps every field. A defensive branch shows Start/End pickers if a multi-day event were ever opened in the (theoretical) student edit path, so stored ranges can never be silently collapsed. No backend schema, endpoint, or engine change was needed or made.
- **UI-040 handling:** `h-full` applied to the five sibling cards (Today already had it) and `mt-auto` on the four CardFooters so the CTA strips pin to the bottom edge of stretched cards — alignment achieved with existing grid/flex behavior, no fixed heights, no truncation; mobile single-column stacking unaffected.
...
## 2026-09-09 — Quiz Eligibility QC-II investigation (chronological)

1. Read the prior audit (docs/QUIZ_ELIGIBILITY_CALCULATION_AUDIT_2026-09-07.md), ADR-010, S4 PRODUCT_SPEC section 5, timetable.json policies/quiz dates, S3.10 baseline; confirmed the frozen Phase 7.3 contract (both criteria = (L%+T%)/2, differing only in window).
2. Traced the full data flow in source: quiz_repo effective dates (active QUIZ_DAY events) -> calendar_engine window pair (C-I prev-quiz-inclusive -> quiz-1; C-II semester commencement -> quiz-1) -> attendance_repo date-bounded scans + collapse_count_rows -> EligibilityService._build_counts/_evaluate_subject (single canonical path for endpoint AND dashboard batch) -> eligibility_engine.evaluate_quiz_eligibility (per-criterion _evaluate_criterion + optimize_attendance; final OR; state; best_opt = min((not reachable, deficit), ties->I)) -> QuizEligibilityCard (presentation-only, toFixed(1) display rounding).
3. Local dev DB was down; started Docker Desktop + the attendancedashpro_db container (data volume untouched) to run the audit; left it running for the user's follow-up testing.
4. Wrote a scratch audit script (temp dir, outside repo): raw-asyncpg independent recomputation (own window logic, own enumeration, own status/cancellation/quiz-day-shape semantics) + in-process app-path execution inside a SET TRANSACTION READ ONLY transaction (rolled back). Two script bugs found and fixed en route (Postgres enum LABELS vs Python enum values: class_type::text returns 'LECTURE'/'ATTENDED' while ClassType.LECTURE.value == 'L').
5. Result: 18/18 exact match on every field, including the top-level OR-route selection (BCS-503 Q2 correctly switches to Criterion II: must 1L+4T (5 total) beats C-I 5L+3T (8)); unreachable routes never win (BCS-054 Q2 C-II best-case 70.48% < 75 correctly loses to reachable C-I).
6. Edge probes: past-unmarked sessions remain "pending"/attendable (BCS-058 Q1: 6 past-unmarked inside must-attend 6L+2T); inclusive previous-quiz-date boundary confirmed in data (BCS-501/BNC-501 lectures on the previous quiz date counted in the next C-I window; quiz-day-shaped sessions excluded); timetable.json quiz dates == DB active events for all six subjects; no NCC/sports/co-curricular provision anywhere in repo; batch == single path.
7. Compared the brief's observed UI values against both snapshots (09-07 audit tables, 09-09 recomputation): not reproducible (BCS-058's quoted C-I row matches Q2/Q3 exactly; its C-II row matches nothing) — flagged as an environment/data-date question, not a calculation error.
8. Findings ranked: 0 critical; H-1 Safe Skip not independently optimized (understates max skips in 4 Q3 windows; always achievable; eligibility unaffected); M-1 selected criterion not exposed; M-2 past-unmarked in must-attend; M-3 observed values unreproducible; 5 LOW. Recommended fixes listed in report section 12 — NOT implemented (investigation-only mandate).
9. Governance: this walkthrough entry, the MASTER_ROADMAP record, task.md replaced with the authoritative current tasks; implementation_plan.md deliberately untouched (no authorized implementation plan exists yet).
10. Read-only discipline: no app code, no migrations, no data writes, no commits, no browser tests. Scratch scripts live in the Windows temp dir.

---

## 2026-09-09 — Phase 27 implementation (Safe Skip independent optimization + route provenance) — chronological

1. Re-read the audit findings and the exact code sections (`eligibility_engine.evaluate_quiz_eligibility` step 7, `EligibilityResult`, `QuizEligibilityCard` guidance boxes, `types/api.ts`).
2. Consumer sweep before touching anything: backend readers of quiz `optimization` are `dashboard_service._build_quiz_snapshot` (`is_reachable` only) and the Phase-1 verifier (field-specific assertions on unchanged fields); `notification_service` uses the unrelated `SubjectAttendanceSummary.optimization` (subject-level 75% optimizer) — untouched. Frontend readers: only `QuizEligibilityCard.tsx` + the type. Conclusion: additive schema fields + engine-internal selection change break nothing.
3. Schema: added `must_attend_criterion` / `safe_skip_optimization` / `safe_skip_criterion` (Optional, default None) to `EligibilityResult` with a full contract docstring; reused `OptimizationResult` as the route object per project conventions.
4. Engine: rewrote step 7 — Must Attend unchanged (same candidate list/key/stable min; only source capture), Safe Skip newly independent (max total skips among reachable criteria, strictly-greater replacement over the C-I-first order = documented C-I tie preference; None when nothing reachable). No new enumeration — each criterion's own optimization is already its max-skip combination (skips = pending − attended).
5. Frontend: three additive fields in `types/api.ts`; card labels now "Must Attend — Criterion I/II" and "Safe Skip — Criterion I/II" (backend-emitted, "(best route)" fallback for legacy payloads); Safe Skip box reads its own route object and hides when absent; its tutorial line value-gated (> 0, CriterionRow convention) to stay correct for a C-II skip route when the C-I window has no tutorials (latent edge, none live). Visual design untouched.
6. Synthetic matrix (drives the real `evaluate_quiz_eligibility`, no DB): 14/14 PASS after correcting two of MY expectations (case 11: C-II min combo (0L,3T) leaves 11 lecture skips, not 8; case 13: max-skip route for an already-eligible pair is genuinely C-II 5 > C-I 3 — the engine was right both times). Covers: only-C-I, only-C-II, both, neither, both-better on each side, cross-divergence in both directions, equal-must tie, equal-skip tie, no tutorials, zero pending, already eligible, per-criterion tie-break unchanged, UNRESOLVED all-None.
7. Live-DB verification (read-only transaction): 18/18 combinations match the audit's independent expectations on state + Must Attend + new Safe Skip/provenance; the four divergence cases now report the true maxima via Criterion II (BCS-503 Q3 17, BCS-058 Q3 7, BCS-501 Q3 8, BCS-502 Q3 6); QC-II windows that were already correct are unchanged.
8. Static checks: py_compile + import OK; tsc --noEmit PASS (0 errors); ESLint PASS on both changed frontend files. No browser/manual testing (user's explicit instruction).
9. Discovered & documented (no code impact): when C-II wins Must Attend it mathematically must also win Safe Skip (pending superset ⇒ more skips), so the cross-divergence only ever presents as C-I-must + C-II-skip; the generic code supports both directions regardless.
10. Governance updated (this entry, MASTER_ROADMAP Phase 27, implementation_plan.md Phase 27 section, task.md). No commit/push/deploy — user's decision.
