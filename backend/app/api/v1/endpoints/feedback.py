...
async def list_feedback_admin(
    ...
    """
    Phase 21B: admin-only paginated feedback list, newest first. STUDENT →
    403 (require_admin); unauthenticated → 401 (auth dependency chain).
    Only the submitter's roll_number/name are joined — no credentials.
    """
...
async def get_feedback_admin(
    ...
    """
    Phase 21B: admin-only single feedback item. 404 when absent; STUDENT →
