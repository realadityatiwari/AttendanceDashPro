...
**Phase 11C-P2 � Push subscription persistence COMPLETE (2026-09-02):** the persistent subscription layer P3's dispatch needs. **Backend:** new `push_subscriptions` table (migration `f0e1d2c3b4a5`, applied to the local dev DB only; alembic head advanced from `a9b8c7d6e5f4`) � `user_id` FK ? users.id (indexed), `endpoint` Text with DB-enforced `UNIQUE(endpoint)` idempotency (multi-device supported: one row per endpoint, many rows per user), `p256dh`/`auth`, IST timestamps, safe downgrade; model + owner-scoped repository (`PushSubscriptionRepository`, pg `INSERT ... ON CONFLICT DO UPDATE`, owner-scoped delete) + service (`PushSubscriptionService`) + endpoints `POST /api/v1/push-subscriptions` (authenticated, idempotent upsert, owner always from the JWT � client `user_id` rejected 422 via `extra="forbid"`, HTTPS-endpoint + key-length validation) and `DELETE /api/v1/push-subscriptions/{subscription_id}` (owner-scoped, cross-user ? 404). **Frontend:** `frontend/src/lib/push.ts` (VAPID public-key config boundary `NEXT_PUBLIC_VAPID_PUBLIC_KEY` � empty placeholder for P3, never a private key; `urlBase64ToUint8Array` + `arrayBufferToBase64Url` helpers), `usePushSubscription(enabled)` hook (user-gesture-only subscription via `enable()`; existing `PushSubscription` reused via `getSubscription()` before `subscribe()` � no duplicate browser subscriptions across tabs; existing-subscription ? backend sync when Settings opens; `disable()` removes backend record + browser subscription with honest failure states; failures contained � never touches auth), Settings "Browser notifications" row now distinguishes permission from subscription readiness (enabled/Disable vs "push setup is incomplete" vs setup error + Retry; honest VAPID-missing state). `npx tsc --noEmit` PASS, ESLint changed files PASS, `compileall` PASS, `verify_phase_11c_p2.py` **24/24 PASS**, migration upgrade/downgrade/re-upgrade verified on the local dev DB. No PushManager.subscribe() path runs until a VAPID public key is configured (P3); no VAPID, no pywebpush, no dispatch, no triggers, no in-app notification change, no auth change, no production migration. No commit/deploy. P3�P5 remain NOT STARTED.

**Phase 11C-P3 � Web Push delivery infrastructure (VAPID + PushDispatchService) COMPLETE (2026-09-02):** the actual Web Push DELIVERY layer. **Configuration:** `VAPID_PUBLIC_KEY` / `VAPID_PRIVATE_KEY` / `VAPID_SUBJECT` added to `Settings` (backend `.env.example`, `deploy/.env.prod.example`, `render.yaml` � all placeholders/empty; private key server-side only, never committed; public key must match the frontend `NEXT_PUBLIC_VAPID_PUBLIC_KEY`). **Dependency:** `pywebpush>=2.5.0` pinned in `requirements.txt` (installed on Python 3.13.5; brings aiohttp + py-vapid + http-ece + requests). **Service:** new `backend/app/services/push_dispatch_service.py` � `PushDispatchService` (async `webpush_async` delivery boundary, injectable for verification) + `PushPayload` dataclass (exact P1 service-worker contract `{title, body, icon, badge, tag, url}` + optional additive `notification_id`/`kind`; same-origin relative URL enforcement) + `PushResult` outcomes (`SUCCESS` / `INVALID_SUBSCRIPTION` / `TEMPORARY_FAILURE` / `CONFIGURATION_ERROR` / `UNEXPECTED_ERROR`) + `DeliveryResult` per subscription. `dispatch_to_user()` / `dispatch_to_subscription()`; provider **404/410 ? permanently-invalid subscription removed** via the existing P2 repository (owner-scoped, repo-owned commit); network errors/5xx/429 ? TEMPORARY_FAILURE with the row kept; missing VAPID ? CONFIGURATION_ERROR with no send attempt. Failure isolation: one subscription's failure never affects others; canonical in-app notifications are never touched; no public dispatch API added (P4 owns triggering); credentials (endpoint/p256dh/auth) never logged. `python -m compileall` PASS, `app.main` import PASS, `verify_phase_11c_p3.py` **20/20 PASS** (configuration validation, payload serialization, mocked success path, 404/410 cleanup, transient non-deletion, multi-subscription isolation, canonical-notification untouched, no credential leakage in logs), `verify_phase_11c_p2.py` **24/24 PASS** regression. Alembic head unchanged (`f0e1d2c3b4a5` � no migration). No frontend changes. No commit/deploy. P4 (triggers) and P5 (bell refresh) remain NOT STARTED.

**Phase 11C-P4 � Canonical Notification ? Web Push Triggers COMPLETE (2026-09-02):** the trigger layer that decides WHEN a canonical in-app notification becomes a Web Push. **GET /api/v1/notifications is now READ-ONLY** � generation-on-read (the 11A/11B architectural problem) is eliminated: `get_notifications()` serves only the persisted inbox from the DB (or the 60s TTL cache) and never generates or writes rows (the historical `verify_phase_11b.py` assertions of generation-on-read and its pinned alembic head are intentionally superseded). **Emission boundary:** new `NotificationService.emit()` � race-free create via `INSERT ... ON CONFLICT DO NOTHING RETURNING` (`NotificationRepository.try_create`), refresh-in-place for existing rows, and best-effort push dispatch ONLY for newly created rows (existing-row refresh never re-pushes; `PushPayload` derived from the canonical row: body=message, notification_id=row.id, kind=row.kind, kind-based same-origin deep link e.g. `/tools/events`, `/history`, `/tools/quiz-schedule`). **Mutation triggers (post-commit, fully isolated, never raise):** attendance marking ? `after_attendance_mutation` re-evaluates ATTENDANCE_THRESHOLD/MUST_ATTEND/SAFE_SKIP for the affected subject (`AttendanceService.record_attendance`); event create/update ? `after_event_mutation` emits ACADEMIC_EVENT to the enrolled-scope recipients with elective resolution (concrete-subject events reach only that subject's enrollees � no anchor/slot leakage; elective-slot events reach slot choosers + anchor fallback; global events broadcast to all users; past/inactive events skipped) (`EventService` � admin portal delegates into it, so its events are covered too); quiz schedule create/update ? `after_quiz_mutation` re-evaluates QUIZ_APPROACHING for enrolled users (+ slot choosers) (`AdminQuizService`). **Sweep:** `NotificationService.regenerate_user_notifications(user)` regenerates ALL kinds (incl. time-dependent CLASS_REMINDER gated by the `class_reminders` preference) � callable, NOT scheduled (no cron/Celery/Redis/APScheduler added); sweep deps: `UserRepository.get_enrolled_user_ids` / `get_elective_choices_for_slot` / `get_all_user_ids`. **Idempotency:** (user_id, kind, occurrence_key) DB-enforced; duplicate trigger = refresh without push. **Failure isolation:** push failure (incl. 404/410 cleanup) never deletes/alters the canonical row; domain mutation success + notification success + push failure still returns success; missing VAPID = side-channel CONFIGURATION_ERROR only. No public send-push API; no frontend changes; NO MIGRATION (alembic head `f0e1d2c3b4a5` unchanged). `compileall` PASS, `app.main` import PASS, `verify_phase_11c_p4.py` **27/27 PASS** (read-only GET, emit creates row, push-once semantics, duplicate suppression, push-failure isolation, zero-subscription + VAPID-missing persistence, recipient isolation, read-state regression), P3 verifier **20/20 PASS**, P2 verifier **24/24 PASS** regressions. No commit/deploy. P5 (bell/in-app refresh) remains NOT STARTED.


**Phase 11C-P4 AUDIT (2026-09-02): REJECTED** — two CRITICAL defects found (F-1/F-2). **REMEDIATION COMPLETE (2026-09-03)** — F-1/F-2 fixed in notification_service.py, verifier extended to 38/38 with direct trigger-path coverage (P5-P8). P4 is ready for re-audit/acceptance. P5 remains NOT STARTED.

**Phase 11C-P5 — Notification Bell / In-App Refresh COMPLETE (2026-09-03):** frontend refresh/presentation layer. Newly persisted canonical notifications (P4 triggers) become visible in the bell badge and notification center without a full-page reload. **Service-worker signal:** the P1 push handler now posts `{ type: "NOTIFICATIONS_UPDATED" }` to all controlled window clients after showNotification (invalidation hint only, no secrets, no reconstructed inbox). **Frontend listener:** new `NotificationRefreshListener` in AppShell validates the message and triggers targeted SWR revalidation of the canonical notification key (no other cache touched, no page reload, no polling, cleanup on unmount). **Shared key:** `NOTIFICATIONS_KEY` exported from `frontend/src/lib/api.ts`; bell, center, SW signal, and PATCH all target the same cache entry. **Foreground recovery:** preserved the pre-existing STANDARD_CACHE focus revalidation (60s dedupe) on the always-mounted bell key, so background-to-foreground catches up with one deduped request; no manual visibility listener added, no request storm. **No polling added.** GET /notifications remains read-only; P4 persistence/emit untouched; no backend/migration/auth changes. `npx tsc --noEmit` PASS, eslint PASS (0 errors), `node --check service-worker.js` PASS, compileall PASS, alembic head `f0e1d2c3b4a5` unchanged. Manual browser/PWA testing is the user responsibility. P5 implementation complete; P5 manual E2E PENDING.

**Phase 11C Production Verification & Hardening (2026-09-04, verification-only -- zero code/deploy changes):** production Web Push chain verified as far as safely possible. **Production Alembic `f0e1d2c3b4a5`** (P2 push_subscriptions migration applied 2026-09-04, after a fresh pre-migration pg_dump in `production-backups/`); production `push_subscriptions` table present with 1 persisted subscription (browser POST /push-subscriptions 200 verified by the operator; OPTIONS 200 + unauthenticated POST 401 re-probed read-only). **VAPID presence verified without values:** Render backend triple (`VAPID_PUBLIC_KEY`/`VAPID_PRIVATE_KEY`/`VAPID_SUBJECT`) + Vercel `NEXT_PUBLIC_VAPID_PUBLIC_KEY` confirmed configured by the operator; deployed bundle independently verified -- the public key is inlined at the exact `VAPID_PUBLIC_KEY=(...).trim()` boundary of `lib/push.ts` (87-char base64url; fingerprint-only inspection, value never printed). **P1/P5 deployed service worker verified:** deployed `/service-worker.js` contains the `push`, `notificationclick`, and `NOTIFICATIONS_UPDATED` handlers. **Static P5 chain re-verified:** SW signal -> AppShell-mounted `NotificationRefreshListener` -> targeted `mutate(NOTIFICATIONS_KEY)`; no polling, no broad visibility listener; bell/center/PATCH share the canonical key. **Reachability guard 5/5 PASS** (`verify_prod_reachability.py`). **Actual Web Push delivery NOT executed** -- no safe existing runtime mechanism exists to invoke `PushDispatchService` in production (no one-off shell, no dispatch API by design), and no permanent test endpoint may be created; delivery verification remains a manual/runtime gap. Safest next step: a one-off, operator-executed runtime invocation (e.g. Render one-off job or ephemeral console) running the existing `PushDispatchService.dispatch_to_user` with a harmless payload -- never a committed endpoint. Remaining manual checks: foreground/background notification display, notificationclick deep link, P5 bell refresh on push, INVALID_SUBSCRIPTION cleanup. No commit/push/deploy; no secrets printed; no schema mutation; temp verification script removed after use (git tree clean except the operator's backup dump).
...
- **UI/UX Remediation — PLANNING COMPLETE (2026-09-05, planning-only, no implementation)** -- The student-facing UI/UX audit (`docs/UI_UX_AUDIT_STUDENT_APP_2026-09-05.md`, 40 findings: 2×P0 / 7×P1 / 17×P2 / 11×P3 / 3×P4, six systemic root patterns) has been transformed into the implementation-ready blueprint at `docs/UI_UX_REMEDIATION_BLUEPRINT_2026-09-05.md`: 11 remediation units, 12 dependency-ordered phases (Phase 0 = decision gate), finding→remediation matrix covering all 40 findings, 13 product decisions (D-01..D-13) requiring user approval, design-system consolidation plan, navigation/IA proposal (routes unchanged in all options), feedback/error model, responsive strategy, accessibility plan, terminology standard, file impact map, risk matrix, manual + static verification plans, and protected-systems list (auth/session, attendance engine, eligibility, notification generation, Web Push, API contracts, schema/migrations all untouched by plan). TWO P0s verified against compiled CSS: dead `surface`/`surface2`/`text2` tokens (invisible skeletons) and `--accent` #262626 used as visible color (invisible calendar indicators). Planning status only — NO remediation phase has started; no code/CSS/routes/copy modified; no commit.

- **UI/UX Remediation Phase 1 — Visual Foundation COMPLETE (2026-09-05)** -- Implemented per the approved blueprint (Phase 1 scope ONLY: UI-001 dead tokens + UI-002 accent misuse; no P1/P2/P3/P4 items touched). UI-001: all 31 dead-token usages (`bg-surface`, `bg-surface2`, `text-text2` — tokens defined nowhere) migrated to EXISTING canonical tokens with zero new tokens invented: subtle elevated surfaces/skeletons/chips/avatars → `bg-muted` (with intended opacity variants `bg-muted/50`, `/30` preserved), tab/recessed tracks → `bg-muted`, card-like empty panel → `bg-card`, secondary text qualifier → `text-muted-foreground/70`. Skeleton P0 root cause fixed at the class level: `cn("bg-muted", "bg-surface/50")` previously let tailwind-merge drop the only valid background for an invalid one (invisible skeletons); `bg-muted/50` now wins the merge AND compiles (`.bg-muted\/50` verified in built CSS). 12 files: laboratory page (8 sites incl. tab track, progress track, row hover, number chip), quiz-schedule page (info panel, inactive tabs, GlassCard skeleton), SubjectAttendanceGrid (loading grid + empty panel), SubjectLaboratoryView (6 sites), QuizEligibilityCard (5 sites), OverallAttendanceCard (`text-text2`), UserMenu + ProfileModal + AdminShell avatars. UI-002: all 13 VISIBLE accent-as-color usages repointed to semantic tokens with identical opacity modifiers (accent→primary mapping): calendar event dot/count + legend Event swatch, day-cell selection ring/tint + legend Selected swatch, DayDetail substitution line/"View all" link/event-item left border, EventRow "Calendar" link, profile section icons, quiz intro Info icon → `primary`; feedback page SUGGESTION badge `bg-accent/10 text-accent` → `bg-warning/10 text-warning` (only remaining semantic slot; BUG=destructive/QUESTION=primary/PRAISE=success retained). Accent SURFACE usages intentionally retained (shadcn hover/focus semantics): UserMenu trigger `hover/focus/data-popup-open:bg-accent`, dropdown-menu item `focus/data-open:bg-accent` + `accent-foreground` text. `--accent` token value itself UNCHANGED (no hover-surface repaint). Calendar behavior/logic untouched (class swaps only); today's primary circle + `ring-primary/50` unchanged; selected = `bg-primary/10 ring-2 ring-primary` (ring width/color groups override today's thinner ring via tailwind-merge when both apply; today's filled circle remains the today marker). Verified: `npx tsc --noEmit` PASS; ESLint PASS on all 14 changed files; `npm run build` (CI placeholder NEXT_PUBLIC_API_URL) PASS 25/25 pages; compiled CSS contains ZERO dead-token selectors and ALL corrected utilities (`.bg-muted\/30|\/50`, `.bg-card`, `.bg-primary\/10|\/15`, `.ring-primary`, `.border-primary`, `.text-primary`, `.text-muted-foreground\/70`, `.bg-warning\/10`, `.text-warning`); `git diff --check` PASS; frontend diff = 42 insertions / 42 deletions, pure class swaps; `git diff` on `backend/` EMPTY (no schema/migration/auth/engine/contract change). During implementation one self-inflicted paren drop in laboratory/page.tsx (map closers) was caught by tsc and repaired — final file differs from pre-phase state ONLY in the 8 intended class lines. No commit/push/deploy. Remaining for user: manual browser pass on skeleton visibility (Laboratory/Quiz/Subjects), calendar indicator visibility at all states (event dot, selection, today, legend correspondence), avatar/info-panel tints.

---

# Phase 27 -- Local Development Environment

- **27.1 Local dev backend port migration 8080 -> 8300 COMPLETE (2026-09-05)** -- Windows on this dev machine holds TCP 8027-8126 inside a Hyper-V/WinNAT excluded port range, so the dev backend could no longer bind 8080 (WinError 10013); 8300 verified free beforehand (no netstat listener, no netsh excludedportrange coverage). Migrated the LOCAL DEVELOPMENT workflow only -- production untouched (Render injects PORT; Dockerfile default 8000; docker-compose maps only postgres 55432): `start-dev.ps1` `$backendPort = 8300` + rationale comment (availability/ownership checks, bind wait, error messages, and the displayed URLs all derive from `$backendPort`, so the script stays internally consistent by construction); `stop-dev.ps1` header comment + `Stop-DevPort -Port 8300`; frontend local dev API target -> `http://127.0.0.1:8300` in `frontend/.env.local` (active gitignored local config; 127.0.0.1 over `localhost` per docs/login_connectivity_investigation.md -- `localhost` can resolve to ::1 and miss the IPv4-only bind) and `frontend/.env.example`; `frontend/src/lib/api.ts` `DEV_API_URL` fallback (production guard unchanged: production builds still refuse a localhost API URL); `backend/app/core/config.py` dev-architecture comment only. CORS UNTOUCHED (`BACKEND_CORS_ORIGINS` lists the frontend's 3100 origins; the backend's own port is not a CORS concept) and the cross-site refresh-cookie model is unaffected (both ends remain loopback). Remaining `8080` occurrences are historical only: legacy `npx serve . -p 8080` static-frontend era docs (02_TECH_STACK, 12_PWA_AND_DEPLOYMENT, 14_TESTING_AND_QA, 17_AI_HANDOFF, AUTH_REGRESSION_RECOVERY, S3.5_UI_UX_AUDIT, S3.10_CURRENT_SEMESTER_BASELINE) and dated investigation/audit reports (login_connectivity_investigation, database_driver_mismatch_investigation, bugfix/cancellation_state_and_counting_consistency_report, phase_18_0_infrastructure_audit, phase_21d1_config_hardening), plus `repomix-output.xml` (generated snapshot artifact) -- none are read by any script or runtime path. Pre-existing defect discovered and fixed during verification: `stop-dev.ps1` was saved BOM-less, so Windows PowerShell 5.1 ANSI-decodes its UTF-8 box-drawing/em-dash characters and the parser fails ("Missing closing '}'") -- identical failure confirmed on the HEAD copy (predates this migration; same failure class as the start-dev.ps1 BOM incident fixed in 26.7); UTF-8 BOM restored (byte-only, no logic change). Verified (static only): PowerShell AST parse OK for both launchers under the PS 5.1 parser; `py_compile` on config.py PASS; repo-wide grep: zero `8080` references remain in any executable/config path. Runtime NOT exercised (operator runs the dev environment manually).

- **UI/UX Remediation Phase 2 — Feedback & Error-State Foundation COMPLETE (2026-09-05)** -- Implemented blueprint Phase 2 (UI-005, UI-007, UI-009, UI-022, UI-023, UI-025, UI-026) WITHOUT touching destructive-confirmation policy (Phase 3/D-11), navigation, notification generation, or backend. NEW primitives (no external dependency added): `components/feedback/toast.tsx` (ToastProvider/useToast — success/error/warning/info variants, icon + role=status/alert + aria-live, auto-dismiss 5s/8s, manual dismiss with aria-label, max 3 stacked, mobile position above bottom nav `bottom-24` / desktop `sm:bottom-6 sm:right-6`, canonical semantic tokens + tw-animate fade/slide with `motion-reduce:animate-none`) mounted in AppShell around TopNav/main/bottom-nav; `components/feedback/UpdateBanner.tsx` (role=status, explicit Refresh = user-initiated `location.reload()` — no auto-reload, session dismissal, popover tokens, above bottom nav). UI-005: ErrorState gains OPTIONAL `onRetry`/`retryLabel`/`action` (backward-compatible; admin consumers unaffected); migrated Dashboard (mutate), History (mutate), Subjects grid (ErrorState replaces inline red box; retries subjects+overview mutations), Track (mutate) — retries revalidate the actual failed SWR keys, no polling, cache semantics preserved. UI-007: `app/not-found.tsx` (branded 404, Go to Dashboard) + `app/error.tsx` (client boundary, Try again via reset() + dashboard link, error details console-only) — no loading.tsx added (per scope). UI-022: dashboard renders a non-blocking warning-toned "Some analytics couldn't be loaded — forecast and weekly trends may be missing" note with Try-again targeting ONLY the analytics SWR key when `useAnalyticsOverview` errors while the summary succeeds; primary cards unaffected; early-return summary-error branch unchanged. UI-023: `useServiceWorker` now exposes `updateAvailable` (set when an installing worker reaches installed with an existing controller — the previous console.log branch); ServiceWorkerRegistration renders UpdateBanner when set; SW caching/skipWaiting/push architecture untouched. UI-009: events deactivation error path replaced `window.alert(message)` with `toast({variant:"error", title:"Couldn't deactivate the event", description: message + " Please try again."})`; deactivation/save SUCCESS toasts added ("Event deactivated"/"Event saved"); confirmation semantics unchanged (inline Confirm/Cancel swap retained for Phase 3 to evolve). UI-026: apiFetch's genuine-401 branch (after permanent refresh failure — transient/network paths untouched) now sets idempotent `sessionStorage['session_expired']='1'` before the existing redirect; login page reads it once, shows "Your session has expired. Please sign in again." (warning-toned, role=status), clears the flag; concurrent 401s cannot duplicate the notice. Lint hygiene in touched file: login `catch (err: any)` → typed unknown handling + `Don&apos;t` escape (both pre-existing errors on HEAD). VERIFIED: `npx tsc --noEmit` PASS; ESLint on 15 changed files — 13 clean, remaining 3 findings confirmed PRE-EXISTING on HEAD (history set-state-in-effect ×2 — refactoring those effects out of scope; api.ts no-location-assign warning — frozen auth redirect pattern); `npm run build` (CI placeholder) PASS 25/25 pages; `git diff --check` PASS; grep gates: zero `window.alert`/`alert(` in student scope + components, zero toast-library additions, analytics/SW/session wiring present. SCOPE INTEGRITY: zero API payload/contract changes (grep on diff), zero destructive-confirmation additions, zero navigation changes, backend untouched BY THIS PHASE — NOTE: the working tree also carries the user's own pre-existing dev-port migration (8080→8300 in `backend/app/core/config.py`, `lib/api.ts` DEV_API_URL, `start-dev.ps1`, `stop-dev.ps1`); these pre-date Phase 2, were preserved untouched, and are NOT part of this phase's diff. No commit/push/deploy. Manual browser pass remains: forced error retry, invalid route, runtime boundary, analytics-failure note, deactivation toast, SW update banner, expiry notice, toast mobile/desktop behavior, keyboard access.

- **UI/UX Remediation Phase 3 — Destructive-Action Safety (RU-3) COMPLETE (2026-09-05)** -- Implemented D-11 policy + UI-006/UI-015/UI-009 on top of the Phase 2 feedback foundation. NEW shared primitive `components/feedback/ConfirmDialog.tsx` built on the EXISTING Dialog primitive (no second modal framework, no new dependency): accessible title/description via DialogTitle/DialogDescription, explicit Cancel + operation-specific Confirm (never generic Yes/OK), destructive-vs-default confirm variants, pending lock (confirm+cancel disabled, spinner) preventing double-submission, closes only after the mutation promise settles; caller owns mutation semantics and feedback. UI-006 (Track bulk): "Mark all present" button now ONLY opens ConfirmDialog showing the REAL pending count ("Mark N class(es) present?") with recoverability wording ("You can still change each class afterwards"); the initial tap never mutates; no optimistic update; result feedback is honest with real counts from Promise.allSettled — full success toast "Marked N class(es) present", partial "Marked X of Y classes present / Z could not be updated" (warning) + existing inline error, total failure error toast + "No classes could be marked present"; existing isMarkingAll double-submit guard preserved. UI-015 (Laboratory): BOTH student-facing destructive actions now confirm — delete-record ghost Trash2 button opens ConfirmDialog ("Delete this record?" naming experiment number/subject, recoverability stated, non-null record guarded instead of the old `!` assertion) and admin deactivate-experiment opens ConfirmDialog ("Deactivate this experiment?" naming experiment/title/subject); `run()` now returns success boolean so success toasts ("Record deleted"/"Experiment deactivated") fire ONLY on genuine success; failures keep the existing inline error banner (toast not duplicated); payloads/endpoints unchanged (same deleteRecord/deleteExperiment calls). UI-009 (Events): inline Confirm/Cancel RETAINED per D-11 (structurally sound); verified no window.alert, success/error toasts from Phase 2 intact; ADDED in-flight ref guard so a rapid double-click on inline Confirm can never submit twice. Notification dismissal NOT touched (Phase 8). Admin-portal `alert()` usages remain out of scope (reported as residue). VERIFIED: `npx tsc --noEmit` PASS; ESLint on 4 changed files PASS (0 findings); `npm run build` (CI placeholder) PASS 25/25 pages; `git diff --check` PASS; static scope greps: 0 student-scope alerts, Dialog primitive reused, mutation payloads/endpoints unchanged (grep on diff), notification center untouched by Phase 3, backend untouched BY THIS PHASE (backend/ working-tree diff remains only the user's pre-existing 1-line dev-port change in config.py documented in Phase 2), migrations untouched. Phase 3 diff: ConfirmDialog.tsx (new) + 3 modified files (131+/16−). No commit/push/deploy. Manual browser pass remains: bulk confirm/cancel/double-submit honesty, lab delete+deactivate confirmations, events inline confirm with guard, keyboard/mobile dialog behavior.

- **UI/UX Remediation Phase 4 — Navigation, IA & Responsive Shell (RU-4) COMPLETE (2026-09-05)** -- Implemented D-01..D-04 + UI-003/UI-004/UI-008. NEW shared source of truth `components/layout/navItems.ts`: every destination defined once with canonical D-01 labels ("Mark Attendance" for /tools/laboratory, "Lab Experiments" for /laboratory), icons, and mobile page titles, plus role helpers (navItemsForRole/moreItemsForRole/mobileTabItems) and group metadata (MOBILE_TAB_HREFS unchanged IA; MORE_HREFS = Mark Attendance, Lab Experiments, Quiz Eligibility, Calendar, Events [+admin Feedback]). TopNav now renders THREE bands: lg+ = full navigation inline (existing design); md-lg = primary destinations inline + secondary under an accessible "More" DropdownMenu trigger (active tint + aria-current="true" when a secondary route is open, chevron, items render as Links with aria-current) — the md-band horizontal overflow risk (UI-003) is eliminated structurally, no font shrinking, no scrolling, no clipping, route availability unchanged; below md = nav hidden (mobile bottom bar owns wayfinding). Mobile header (D-04): for every non-home route the header shows the current page title (centered, truncating) in place of the brand wordmark below sm; Home/dashboard keeps the wordmark + greeting — no duplicate Home entry anywhere. MobileBottomNav (UI-008): derives tabs/More from the shared data; the "More" control now highlights (text-primary) and carries aria-current="true" whenever the current route belongs to the More group; More-sheet rows keep per-item active state; More aria-label/expanded preserved. UI-004: Track page h1 + error-branch PageHeader now "Mark Attendance"; Laboratory page PageHeader now "Lab Experiments"; routes/URLs untouched (all 10 verified present). D-02: dashboard Attention card CTA retargeted from "View Strategy"→/tools/laboratory to "View plan"→/tools/quiz-schedule (the must-attend/safe-skip surface). Route matching remains exact-match per the existing convention — correct here because no student nav route has nested child routes. Residual observation (documented, not a defect): an ADMIN at exactly lg (1024) sees 9 inline items — strictly better than pre-Phase-4 (9 items at 768) but worth a manual look at 1024-1100. VERIFIED: `npx tsc --noEmit` PASS; ESLint on 6 changed files PASS (0 findings); `npm run build` (CI placeholder) PASS 25/25 pages; `git diff --check` PASS; static searches: zero "View Strategy"/"Track Attendance"/label:"Track"/label:"Laboratory" remaining in frontend/src, "View plan" present with /tools/quiz-schedule target, navItems.ts consumed by BOTH nav components, all 10 route files present (no route URL changed), notifications dir untouched, no backend/migration changes (backend tree still carries only the user's pre-existing config.py port line), no Phase 5 container refactor (AppShell diff remains the Phase 2 ToastProvider mount only). Phase 4 diff: navItems.ts (new) + TopNav, MobileBottomNav, AttentionRequiredCard, Track page, Laboratory page. No commit/push/deploy. Manual pass remains: 768/1024 overflow + More dropdown, lg full nav, Mark Attendance/Lab Experiments labels, View plan target, mobile More-active + header titles at 360/390, route persistence between secondary routes.

- **UI/UX Remediation Phase 4 CORRECTION — Desktop TopNav single-line fix (2026-09-05, manual-verification follow-up)** -- Narrowly scoped fix after the user's manual browser pass exposed a lg+ visual regression: D-01's longer labels (Mark Attendance / Lab Experiments / Quiz Eligibility) grew the nav's intrinsic width ~+75px vs the old labels, and because nav links, the More trigger and the user-identity controls were ordinary shrinkable flex items (nav even had min-w-0 licensing shrink), flexbox compressed them and labels plus the user name wrapped to two lines. ROOT CAUSE was flex squeeze, not font/padding choices. FIX (TopNav.tsx + one class in UserMenu.tsx only): (1) atomicity — every nav item, the More trigger, and the right control group now carry shrink-0 + whitespace-nowrap; (2) the shrink license removed (min-w-0 dropped from both navs); (3) the identity span got whitespace-nowrap; (4) modest sanctioned tightening to reclaim exactly the label growth so lg+ returns to old-baseline width parity: item padding px-3→px-2.5 (−64px across 8 items), nav gap lg:gap-1.5→gap-1 (−14px), header gap-4→gap-3. NO clipping/overflow-hidden/truncation on desktop, NO font-size change, NO horizontal scrolling, NO breakpoint moves, NO mobile changes (page titles/More active state/bottom nav untouched), navItems.ts untouched, D-01/D-02 labels and D-03/D-04 architecture intact. Verified: tsc PASS; ESLint (TopNav, UserMenu) PASS; build PASS 25/25 pages; git diff --check PASS; diff review confirms only the class-level changes above. Manual re-verification requested at 1024/1100/1280/1440/1600+ (single-line items, no name wrap, no horizontal scrollbar, active states, More dropdown at 768–1023, mobile unaffected). No commit/push/deploy.

- **UI/UX Remediation Phase 5 — Layout Contract & Container Consolidation (RU-5) COMPLETE (2026-09-05)** -- UI-011 resolved: AppShell is now the SINGLE owner of global content width/centering/horizontal padding for authenticated pages (`max-w-5xl mx-auto p-4 pb-28 md:p-6 lg:p-8` — unchanged, verified already authoritative; no shell adjustment needed). All 17 page-root shell-duplication occurrences across 9 files removed: Calendar ×3, History, Laboratory, Subjects, Events (their max-w-5xl equaled the shell width or max-w-7xl was unreachable inside it — pure duplication, removed with the duplicated px-4/sm:px-6/lg:px-8), plus the padding-only removal on Track ×2, Quiz ×2, Feedback ×2, Profile ×3. LEGITIMATE LOCAL CONSTRAINTS PRESERVED per blueprint §7: Track's max-w-2xl mx-auto (sanctioned narrow reading column), Quiz/Feedback/Profile max-w-4xl mx-auto (narrower-than-shell reading widths) — these now constrain content INSIDE the shell column instead of re-creating the shell. VERTICAL SPACING PRESERVED everywhere (py-6/py-8, gap-6, space-y-6, PageHeader mb-8, GreetingHeader mb-8 untouched) per the horizontal-only rule; no double vertical-padding "fixes" attempted. Net effect: horizontal inset on mobile drops from 64px (shell 32 + page 32) to 32px; desktop from 128px to 64px; content widens by the previously-wasted padding (expected, documented). Login/signup untouched (own centered layout outside AppShell — not shell duplication); dialogs/inputs/cards with local max-w untouched; admin portal out of scope; dashboard root was already clean (w-full only). VERIFIED: tsc PASS; ESLint 9 changed files — 8 clean, only the 2 PRE-EXISTING history set-state-in-effect findings (proven on HEAD in Phase 2; this phase touched only that file's root class); build PASS 25/25 pages (CI placeholder); git diff --check PASS; post-change container audit: ZERO px-4 py-/sm:px-6/lg:px-8 shell-duplication patterns remain in app/(authenticated); remaining root max-w-* are exactly the 4 classified legitimate local constraints; AppShell container classes byte-identical; navigation files untouched by this phase (their working-tree diffs are Phase 4); notifications/contexts zero diffs; backend tree still carries only the user's pre-existing config.py port line. No commit/push/deploy. Manual pass: per-page centering/margins at 360/390/768/1024/1280/1440/1600+, no double inset, expected content widening, Phase 3 confirm dialogs + Phase 4 nav/mobile titles intact.

- **UI/UX Remediation Phase 6 — Design-System Primitive Consolidation (RU-6) COMPLETE (2026-09-05)** -- Deliberately SMALL, high-confidence consolidation per the user's guardrail (not a DRY blitz). UI-016: canonical control-height vocabulary established as `h-10 sm:h-8` (the Button foundation's existing mobile-first pair; EventFormDialog + Track inputs already used it). NEW shared primitive `components/ui/select.tsx` (native select, Input-matching styling, `data-slot="select"`, w-full default overridable to w-auto next to labels); shared `ui/input.tsx` raised from fixed `h-8` to `h-10 sm:h-8` (also resolves the UI-024 primitive touch-target half sanctioned for 6.1). ALL 11 student-facing native selects migrated onto Select (history ×2, events ×2, EventFormDialog ×5, Settings ×1, Laboratory ×1), deleting the four accidental height systems (h-8 filter strings, h-10/sm:h-8 dialog string, h-9/sm:h-7 settings, h-9 laboratory); Track's date input dropped its now-redundant height classes; date Inputs across history/events/EventFormDialog reduced to `[color-scheme:dark]`. Remaining h-7/h-9 hits audited: two compact Reset ghost buttons (intentional, retained), a TrackSessionCard comment, a Skeleton (not controls). UI-021 (single consolidation): `humanizeEventType` + `classTypeLabel` — byte-identical copies in EventRow and DayDetail — moved to the existing shared `components/events/eventRules.ts`; consumers (events page, EventFormDialog) re-pointed; EventRow's now-unused ClassType import removed. INTENTIONALLY RETAINED with justification: dashboard `status.ts` classTypeLabel (different contract: string return/"Practical" default vs null), date-chip variants (EventRow/HistoryRow rounded-full vs UpcomingEventsCard rounded-md — minor style drift, no behavioral gain), date formatters (Phase 7/D-09 owns formatting), QUIZ_LABELS vs quiz-page CYCLES (different shapes), remaining inline empty-states (Phase 2 documented contextual variations), EventFormDialog read-only class-type display box (local one-off). UI-009 confirmation-consistency remainder: audited — student-scope DialogFooter users are only EventFormDialog (a form, not a confirmation) and the primitive itself; Track/Lab already use ConfirmDialog; EventRow inline confirm retained per D-11 → NOTHING to migrate (audit clean). UI-032: all 50 `text-[10px]` occurrences in student scope raised to `text-[11px]` (the blueprint's caption floor) across 15 files — form labels, stat labels, date-chip months, badges, metadata, the bell counter; `leading-none` added to the 14 `h-4` badge variants so 11px text does not clip inside the 16px overflow-hidden box; h-5 badges needed no adjustment; `text-[0.65rem]` mobile bottom-nav labels intentionally retained (compact tab microcopy, outside the literal text-[10px] audit). VERIFIED: tsc PASS (2 transient import slips caught and fixed during the pass); ESLint on 20 changed files — 19 clean, only the 2 PRE-EXISTING history set-state-in-effect findings (proven on HEAD in Phase 2); build PASS 25/25 pages (CI placeholder); git diff --check PASS; audits: 0 native selects left in student scope, 0 text-[10px] left, single humanizeEventType definition, ConfirmDialog usages intact + EventRow inline confirm intact (2 hits); diff review: zero handler/payload/onChange changes (grep clean), zero new "use client" directives (client/server boundaries preserved), zero Phase 7 formatting/vocabulary leakage. Phase 6 diff: 18 files, 128+/124− (mostly className swaps + one new primitive). No commit/push/deploy. Manual pass: control sizing across all forms/filters at 6 viewports (esp. now-40px mobile controls), 11px label legibility, badge fit, Phase 3 confirmations unchanged.

- **UI/UX Remediation Phase 7 — Vocabulary, Status, Dates & Percentages (RU-7) COMPLETE (2026-09-05)** -- Presentation-only phase; SEMANTIC-INTEGRITY AUDIT PROVEN (same data → same calculation → different presentation). D-08: NEW `lib/statusLabels.ts` — `attendanceStatusLabel()` maps the legacy dashboard/analytics bands SAFE→"Healthy", WATCH→"Watch", CRITICAL→"Critical" (unknown/null → "N/A"); the two raw-enum badge sites (OverallAttendanceCard, AttentionRequiredCard) now render mapped labels with their existing variants/colors; SubjectAttendanceCard's Health set already rendered canonical labels (Healthy/Watch/At Risk/Critical) and is unchanged. UI-013: WeeklyAttendanceCard's hard-coded `>= 80`/`>= 60` bar thresholds replaced by named constants WEEKLY_BAR_SAFE_PCT/WEEKLY_BAR_WATCH_PCT (= 80/60, documented as mirroring backend legacy bands in attendance_engine.py) — VALUES UNCHANGED, presentation centralized. D-07: dashboard classStatusLabel now Attended→"Present", Missed→"Absent", "Attendance pending"→"Pending" (Today card); Laboratory page maps the backend lab-domain DATA VALUES "Attended"/"Missed" to "Present"/"Absent" for display only — the `=== "Attended"` comparisons are data checks and are UNCHANGED; legitimate prose ("N of M attended") retained everywhere. UI-017/D-09: NEW shared `formatDateMedium()` ("5 Sep 2026", deterministic English, local T00:00:00 parsing — no UTC shift) in lib/date.ts; migrated ALL locale-dependent/Intl date sites: SubjectAttendanceCard (was device-locale), QuizEligibilityCard, EventRow (+ date chip now formatShortDate matching UpcomingEventsCard/HistoryRow chips), DayDetail, laboratory page, SubjectLaboratoryView (also fixes the pre-existing UTC-parse day-shift risk on negative-offset devices), ProfileModal (was en-GB). formatDayHeader deleted (was a byte-identical duplicate of formatShortDate; 2 consumers migrated); dead formatDayInitial removed. Remaining Intl classified legitimate: calendar month labels (en-US month/year) and history "Logged h:MM AM" time (fixed locale). UI-018/D-10: whole-number percentages on SubjectAttendanceCard (headline + detail rows via retained fmtIntPct; fmtPct deleted), QuizEligibilityCard normal rows (shared formatPct), Laboratory practical stat (formatPct) — while QuizEligibilityCard's CriterionRow inside "View Calculation" KEEPS one-decimal precision (the sanctioned calculation exception); formatDelta ("+2.3 pts") untouched (a signed point delta, not a %-display). UI-038: weekly pending indicator `· 3p` → `· 3 pending` (counts span w-16→w-24 to fit). UI-010: "Student Identity (PostgreSQL)"→"Student Identity"; profile deferred box → "Profile editing isn't available yet / This page is read-only for now."; ProfileModal data-lineage footnote removed; InstallAppModal's two contradictory stale paragraphs + "Tracked in task.md" ×2 → single honest install explanation; SettingsModal VAPID phasing note → "push notifications aren't fully set up yet."; FeedbackModal "fake confirmation" rationale box removed; Laboratory "Ingest" button → "Add"; admin Feedback page description "reviewed by the backend-admin contract" → "Feedback submitted by students.". UI-029: login h1 "Student Portal"→"AttendanceDash Pro" (subtitle becomes "Student Portal"), "Login securely"→"Sign in" ("Signing in..." loading), signup subtitle V2 suffix removed, signup link "Login"→"Sign in" — credential model (13-digit roll number) untouched. UI-036: quiz intro restructured from one wall-of-text paragraph into three scannable lines (eligibility rule → shared-average/window explanation → scope note) — every fact preserved, no calculation touched. VERIFIED: tsc PASS; ESLint on 22 changed files PASS (2 pre-existing signup/login-hygiene items fixed in touched files: signup err:any → typed handling, apostrophe escape); build PASS 25/25 (CI placeholder); git diff --check PASS; audits: 0 raw status enums, 0 locale-dependent dates (remaining Intl = month labels + fixed-locale time, classified legitimate), remaining toFixed = whole-number (toFixed(0)) + the sanctioned quiz-calc toFixed(1), 0 user-facing dev copy (one VAPID mention remains in a code comment only), 0 text-[10px], pending indicator resolved. SEMANTIC-INTEGRITY: threshold literals only moved to same-valued constants; zero arithmetic/predicate/payload/query changes; attendance_status data-value comparisons byte-identical; toDate/T00:00:00 local parsing preserved (no day-shift); lib/date.ts formatPct/toDate untouched. No commit/push/deploy. Manual pass: status labels on dashboard cards, Present/Absent consistency incl. Laboratory badges, dates across 6 surfaces (no day shifts), whole-number percentages vs quiz-calc precision, login/signup terminology, quiz intro readability, weekly pending wording.

- **Phase 7 CORRECTION — Attendance decimal precision restored (2026-09-05, manual-verification follow-up)** -- The user's manual pass showed Phase 7 applied D-10 too broadly: calculated ATTENDANCE percentages lost their one-decimal precision (72.2% rendered as 72%, etc.). CORRECTED RULE (supersedes the Phase 7 whole-number interpretation for attendance values): percentages representing ACTUAL CALCULATED ATTENDANCE/ELIGIBILITY display with ONE DECIMAL (formatPct1, toFixed(1), null → "—"); genuinely categorical/summary displays keep whole numbers. NEW shared helper `formatPct1` in lib/date.ts beside formatPct (decimal logic now lives in ONE place — zero scattered toFixed(1) in student scope). Precision restored on exactly the Phase 7 regression set: SubjectAttendanceCard headline (primaryPct) + details rows (72.2%/70.4%/65.2% style again), QuizEligibilityCard lecture/tutorial/average rows AND CriterionRow (all via the retained fmtPct wrapper delegating to formatPct1 — one decimal everywhere on the card, matching pre-Phase-7 behavior), Laboratory practical Attendance stat. RETAINED whole-number (pre-existing presentation, not a Phase 7 regression): history summary, dashboard Overall/Forecast, AttentionRequiredCard values+forecasts, WeeklyAttendanceCard weekly/best/needs-attention, subject-card Lecture/Tutorial blocks, criterion "required 75%" labels. DISPLAY-ONLY: raw fields, calculations, thresholds (80/60 constants + backend bands), payloads, queries untouched — verified via diff grep (zero calculation-side lines). VERIFIED: tsc PASS; ESLint 4 changed files PASS; build PASS 25/25 (CI placeholder); git diff --check PASS. No commit/push/deploy. Manual re-check: subject cards show one-decimal values; quiz rows + View Calculation show one decimal; laboratory stat one decimal; dashboard/history remain whole numbers.

- **UI/UX Remediation Phase 8 — Notification Center & Inbox UX (RU-8) COMPLETE (2026-09-05)** -- UI-014 + UI-039 + D-12 + the notification-dismiss undo portion of D-11, frontend-only; notification generation, PushDispatchService, VAPID, push subscriptions, schema, service-worker push architecture, and the canonical NOTIFICATIONS_KEY SWR architecture all UNTOUCHED. UI-014: the global pending lock (pendingId !== null disabling every row) is REPLACED by per-row pending state (`pendingIds: string[]` + separate `markAllPending`) — unrelated rows stay fully interactive while one row's PATCH runs; only the acting row's Read/Dismiss controls disable (with its own spinner); guards prevent same-row double-submission and row-vs-mark-all races; two rows may mutate concurrently (independent idempotent PATCHes). D-12: "Mark all read" implemented with the EXISTING per-notification PATCH — sequential for..of over the currently unread ids (type-narrowed), NO bulk endpoint, NO migration, NO backend change; per-row cache updates only after each genuine 2xx; failures leave those rows unread (server truth); honest outcome feedback via Phase 2 toasts — full success ("Marked N notifications read"), partial (warning "Marked X of N read / M couldn't be updated" + inline error banner), total failure (error toast + inline banner, no false success). Mark-all control: outline button with CheckCheck icon above the list, visible only when unreadCount > 0, disabled while mark-all runs or any row mutation is in flight. Dismiss-undo (D-11 portion): dismissal stays IMMEDIATE (no confirmation dialog); an 8s info toast offers "Undo" — backed by the SAME idempotent PATCH with is_dismissed:false plus one canonical `mutate()` revalidation (identical mechanism to the service-worker invalidation signal; no polling, no new listeners); undo failure = explicit error toast, server truth preserved. Mark-read undo deliberately deferred (row remains visible; low stakes — documented). Toast primitive extended ADDITIVELY (optional action {label, onClick}; clicking runs the handler and closes the toast) — no new dependency, existing dismiss/aria/timer behavior unchanged. UI-039: notification empty-state icon corrected CalendarDays → Bell (existing lucide icon; CalendarDays retained for the ACADEMIC_EVENT kind badge); structure/typography/spacing unchanged. VERIFIED: tsc PASS (3 type errors from string|null notification_id caught and fixed via guard/type-predicate/narrowed rowId); ESLint (2 changed files) PASS; build PASS 25/25 (CI placeholder); git diff --check PASS; audits: sequential PATCH confirmed (for..of, no Promise.all burst), zero backend bulk-route/mark-all/bulk grep hits, payloads limited to existing {is_read}/{is_dismissed} contract, canonical key + NotificationRefreshListener + NotificationBell intact, no polling added, pwa/api/hooks untouched by THIS phase (their working-tree diffs are Phase 2), backend tree still only the user's pre-existing config.py port line. Phase 8 diff: NotificationCenter.tsx (+~140/−18) + toast.tsx (additive action support). No commit/push/deploy. Manual pass: mark-all full/partial/total outcomes, per-row interactivity during pending, dismiss→Undo restore, empty state after dismissing last row, bell badge sync, 360px layout, undo toast timing.

- **UI/UX Remediation Phase 9 — Settings Behavior (RU-9) COMPLETE (2026-09-05)** -- UI-019 resolved via D-05 + D-06; frontend-only. D-05: week_starts_on now DRIVES CalendarGrid — new optional `weekStartsOn?: WeekStart` prop (default "MONDAY", matching the verified backend default in preferences.py); Monday-start rotates the weekday header columns (Mon..Sun, short labels likewise) and shifts the leading blank count to `(getDay()+6)%7` so the 1st lands under its true weekday; Sunday-start preserved as before. Dates, selection, today, event/attendance indicators stay attached to their real calendar dates (grid fills sequentially after blanks; no date math changed, no new Date parsing, Phase 7 local T00:00:00 handling intact). Calendar page consumes the preference via the SHARED usePreferences SWR key (deduped with Settings — no duplicate requests) and passes `preferences?.week_starts_on ?? "MONDAY"`. D-06: discovery CONFIRMED no automatic attendance marking exists anywhere (auto_mark appears only in the preference model/repo/endpoint — pure storage); therefore NO engine/service/repository/API changes and NO fake functionality. The Settings toggle is kept (existing contract preserved, still persists) with an honest caption: "Automatic attendance marking is not currently active — the preference is saved with your account." The bottom info box was updated for accuracy ("Week start controls the calendar layout. Auto-mark present is saved with your account but does not mark attendance yet.") — the old text claiming saving "does not change calendar calculations" became stale the moment D-05 landed. Settings payload/PUT contract, field names, and auth untouched. VERIFIED: tsc PASS; ESLint (3 changed files) PASS; build PASS 25/25 (CI placeholder); git diff --check PASS; audits: week-math rotation/blank formulas correct for both starts, no new Date() constructions in CalendarGrid, preferences draft payload unchanged (0 payload diff lines), backend diff still only the user's pre-existing config.py port line, engines/services untouched, no migration. Phase 9 diff: CalendarGrid.tsx, calendar/page.tsx, SettingsModal.tsx. No commit/push/deploy. Manual pass: Monday vs Sunday layouts (month starting Fri/Sat/Sun esp.), selection/today/indicators under both starts, settings save → calendar reflects choice, auto-mark caption, 360px settings layout.

---

## 2026-09-09 — Quiz Eligibility QC-II Investigation (read-only audit; report: docs/QUIZ_ELIGIBILITY_QC2_AUDIT_2026-09-09.md)

**Status:** COMPLETE. No application code changed; no implementation plan created (no fix authorized yet — implementation_plan.md intentionally untouched).

Independent recomputation from raw tables (raw asyncpg, zero app-engine imports) vs the real app path (in-process, read-only rolled-back transaction) across all 18 subject/cycle combinations (6 theory subjects x Quiz I/II/III): **18/18 exact match** — windows, L/T counts, percentages (full precision), 70/75/75 thresholds, per-criterion pass/fail, per-criterion exhaustive Must-Attend/Safe-Skip enumeration, top-level OR-route selection, final `C-I OR C-II` decision, canonical state. Batch path == single path. Frontend is presentation-only. The engine NEVER mixes C-I and C-II inputs; Must Attend is optimal under the OR rule.

Findings (details + code locations in the report): **H-1** Safe Skip not independently optimized — displays the min-Must-Attend route's skips instead of max skips across reachable criteria (4/18 windows diverge today, all Quiz III: BCS-503 Q3 shows 5 vs true max 17; BCS-058 Q3 4v7; BCS-501 Q3 5v8; BCS-502 Q3 5v6; no Quiz II divergence in current data; displayed value always achievable, never overstates). **M-1** the selected criterion for the top-level guidance is not exposed in the API/UI ("best route" is unnamed — traceability is inferential). **M-2** Must Attend includes past-unmarked sessions as attendable (back-markable-by-design; guidance does not distinguish, e.g., BCS-058 Q1 must 6L+2T of which 4L+2T already past on 09-09). **M-3** the observed UI values quoted in the brief (BCS-503 C-I 16.7%/C-II 65.0%, BCS-058 C-II 47.5%, BCS-501 1L+1T/3L+1T) match NEITHER the 09-07 nor the 09-09 local-DB snapshots — likely a different environment/data date (production never accessed). LOW: latent tutorial-only inconsistency (avg None vs reachable optimizer); unreachable-criterion guidance hidden by D1 (backend does compute full C-II optimization — verified in every response); dead combined_threshold; inactive QUIZ_DAY event residue.

Institutional comparison: thresholds, OR structure, and window anchoring match ADR-010/S4 product spec section 5; the (L%+T%)/2 formula for both criteria is a frozen PROJECT decision (Phase 7.3) — NOT verifiable against the notice (notice not in repo); C-II anchored at semester commencement is equivalent to "subject's first class" for this dataset; NO activity/NCC/sports/co-curricular attendance provision exists anywhere in the repo or engine — if the notice grants such provisions toward quiz eligibility, the engine does not model them. Open clarifications recorded in report section 13.

Verdict: **NEEDS FIXES** (guidance optimality/traceability) — core mathematics CORRECT, no critical logic error.

---

## Phase 27 — Quiz Eligibility Safe Skip independent optimization + route provenance (QC-II remediation) — 2026-09-09 — COMPLETE

**Source:** docs/QUIZ_ELIGIBILITY_QC2_AUDIT_2026-09-09.md finding H-1 + M-1 (user-authorized implementation). Scope guard honored: activity/NCC/sports provisions and the (Lecture% + Tutorial%)/2 averaging formula were NOT touched; eligibility math, date windows, attendance aggregation, pending semantics, and Must Attend selection are unchanged.

**The bug:** the UI's top-level Safe Skip was the skip pair of the min-Must-Attend route (`best_opt`), not the maximum skips across reachable criteria. Under `Criterion I OR Criterion II` the two optimization problems are independent: Must Attend = fewest future attendances among reachable criteria; Safe Skip = most skippable pending classes while still retaining one complete qualifying route. The audit found 4 live windows where the other criterion permitted strictly more skips (all Quiz III).

**Changed logic (backend, `app/engines/eligibility_engine.py` — `evaluate_quiz_eligibility` only):**
- Must Attend selection PRESERVED exactly: same candidate list, same key `(not is_reachable, lecture_deficit + tutorial_deficit)`, stable min over the Criterion-I-first list (ties prefer Criterion I). Only provenance capture added.
- Safe Skip now selected independently: among criteria whose own optimization `is_reachable`, the one with the most total skips (`safe_skip_lecture + safe_skip_tutorial`); strictly-greater replacement over the C-I-first order so equal skips keep the documented Criterion-I preference (deterministic, no random selection). Within a criterion the min-attendance combination already maximizes its skips (skips = pending − attended), so each criterion's own optimization IS its best skip combination — no new enumeration, no cross-criterion mixing. When NO criterion is reachable, `safe_skip_*` are None (state NOT_ELIGIBLE, UI withholds guidance as before).

**Schema (`app/schemas/attendance.py` — `EligibilityResult`, additive/optional, fully backward compatible):** `must_attend_criterion` ("Criterion I" | "Criterion II"), `safe_skip_optimization: Optional[OptimizationResult]` (full route object; displayed values are its `safe_skip_*`), `safe_skip_criterion`. `optimization` keeps its exact previous semantics and shape; UNRESOLVED cycles leave all route fields None.

**Frontend (smallest sensible change):** `src/types/api.ts` — three additive optional-typed fields. `src/components/quiz/QuizEligibilityCard.tsx` — the two guidance boxes now render "Must Attend — Criterion I/II" and "Safe Skip — Criterion I/II" (backend-emitted provenance; "(best route)" fallback only for pre-Phase-27 payloads); the Safe Skip box reads `safe_skip_optimization` and hides when absent (old payload or no reachable route); its tutorial line is value-gated (`safe_skip_tutorial > 0`, the same convention as the per-criterion CriterionRow) so a Criterion-II skip route with tutorials renders correctly even when the C-I window has none. Visual design/hierarchy untouched; no math in React.

**Verification (all read-only):**
- Synthetic edge-case matrix driving the real `evaluate_quiz_eligibility` (temp-dir script): 14/14 PASS — C-I-only / C-II-only / both / neither reachable; C-I better both; C-II better both; C-I must + C-II skip (the brief's second example); C-II must + C-I skip; equal Must (tie -> C-I); equal Skip (tie -> C-I); no tutorials; zero pending; already eligible; per-criterion combo tie-break unchanged; UNRESOLVED leaves everything None.
- Live dev DB re-verification against the QC-II audit's independent expectations: 18/18 combinations PASS on state + Must Attend + new Safe Skip/provenance. The four audit divergence cases now resolve correctly via Criterion II: BCS-503 Q3 17, BCS-058 Q3 7, BCS-501 Q3 8, BCS-502 Q3 6. QC-II windows confirmed unchanged where C-I was already optimal (BCS-501 4, BCS-502 4, BCS-054 4, BCS-058 4, BNC-501 1) and BCS-503 Q2 unchanged (11 via C-II).
- Static: py_compile + imports OK; `tsc --noEmit` PASS (0 errors); ESLint PASS on both changed frontend files.
- Note: `backend/scripts/verify_phase_1_eligibility.py` assertions target only the unchanged `optimization`/per-criterion fields, so they remain valid.
