...
> 
> **Next phase:** Phase 12 — Mobile / Responsive Experience **IN PROGRESS** (12.0 architecture & implementation-readiness audit ✅ · **12A responsive foundation + mobile navigation ✅** — bottom nav restored below `md` (S4 4-tab contract), More sheet, shell/dialog scroll safety, touch-target foundation; desktop unchanged · **12B Track / Dashboard / Calendar responsiveness ✅** — month-nav overflow fixed, grid cells enlarged at 320, Track date nav fluid + ≥40px controls, session-card collision fixes, dashboard wrap fixes; desktop byte-identical — 12C data-heavy pages NEXT). Phase 11 — Notifications & Reminders **COMPLETE & FROZEN** (11.0 architecture audit ✅ · 11A backend notification read model & contracts ✅ · 11B notification persistence + read-state ✅ · 11D notification center UX ✅ · 11E preference wiring verified ✅ · 11F final verification & freeze ✅ — 11C delivery model decision-gated/deferred, not implemented).
>
> **Authorized bugfix executed (2026-08-22) — CLASS_CANCELLED propagation:** real data-integrity defect found during manual testing (active CLASS_CANCELLED events were silent no-ops on recorded/historical classes). Root cause: `EventSessionSynchronizer._reconcile_date` skipped ANY session holding an attendance record. Fixed in the canonical synchronizer (explicit cancellation now propagates over stale marks; restoration always allowed) with consumer alignment via one canonical predicate (`occurrence_is_cancelled`) so cancelled theory classes never count as absences anywhere; LAB_CANCELLED/closure/working-Saturday/quiz-day frozen safety contracts preserved. Verified: new `verify_event_cancellation_propagation.py` **26/26**, compileall PASS, event/session/calendar/attendance verifiers green or stash-A/B-proven pre-existing drift. DB restored to exact baseline counts (alembic head `d1e2f3a4b5c6` unchanged); reported BCS-058 case healed via the canonical path. Details: `docs/bugfix/event_cancellation_propagation_report.md`. **No phase status change — 12C remains next authorized work. No commit made.**
...
## Status: 12.0 + 12A + 12B COMPLETE (2026-08-21); 12C in progress
...
- **12C next** — Laboratory / Subjects / Quiz Eligibility / Events page responsiveness (incl. the Laboratory tab bar ≈380px nowrap overflow fix). No commit made for 12B.
- **Inter-phase authorized bugfix (2026-08-22, COMPLETE & VERIFIED):** CLASS_CANCELLED propagation — see the bugfix note under "Next phase" above and `docs/bugfix/event_cancellation_propagation_report.md`. Phase statuses unchanged; 12B work untouched; 12C still NEXT.
